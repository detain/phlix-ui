...
- **Match popup enrichment with context block (Feature 7)** — `matchSearch()` now returns a `context` block (`original_filename`, `path`, `parsed_title`, `year`, `tags`) on each candidate. `MetadataMatchModal` displays a collapsible "Source info" panel showing the filename (monospace), path (breadcrumb with › separator), and tags (dl/dt/dd grid). The search query box is seeded from `parsed_title` on first response when the user has not edited the query. `MatchContext` interface exported from `@phlix/ui`.
