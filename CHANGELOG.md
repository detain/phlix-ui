...
- **The wire error-code localization layer ships: `src/i18n/errors.ts` (UI W4, error-code doctrine).** The registry lives in `@phlix/contracts` (147 codes / 31 domains at the `v0.5.0` pin — exposed through the main entry as `ERROR_CODES`/`ErrorCode`; the package `exports` map still blocks the `dist/error-codes.json` subpath import, so the typed const is the parity surface and `errors.test.ts` pins the set at runtime against it). This layer maps every registered code to a short user-facing sentence in all seven shipped locales (`en` + es/fr/de/it/pt_BR/ja), each catalog typed `Record<ErrorCode, string>` so a contracts re-pin that adds or removes a code is a COMPILE error until every locale answers. Deliberately a SEPARATE map, NOT `errors.*` keys folded into `DEFAULT_MESSAGES`: the six bundle locales are exact 412-key-identity objects and tizen/windows pin bundle↔installed equality at `#v0.99.5` — registry-adjacent data must not churn every equality pin, and the error layer versions with the contracts pin instead. Accessor `errorCodeMessage(code, locale?, fallback?)` resolves locale catalog → `en` → caller fallback (the server's English debug text) → per-locale generic label, parsing untrusted input at the boundary and never throwing; `errorCodeTitle()` serves the three Browse EmptyState headlines. Active locale arrives config-time like `messages` — `PhlixAppConfig.locale?: PhlixErrorLocale` (additive). Three defects fixed by the doctrine: `LoginForm` showed the server's English on EVERY auth failure in every locale (`auth.error ?? t(...)` put the fallback last) — banner + toast now resolve `auth.errorCode` (new store ref parsed from the `ApiError` body through the existing useLibrariesStore idiom) first, with the code-less server-text path byte-preserved for today's pre-W2 responses; `browseErrors.ts` hardcoded English for `server.relay_unavailable`/`server.no_tunnel`/`server.offline` — now catalog lookups with the former strings shipped VERBATIM as the `en` values (existing pins pass untouched); `PlayerPage` string-matched pseudo-codes in the `error` TEXT field — extracted to pure `playbackBlockingMessage(body, locale?)` reading `body.code ?? legacy text-match`, correct before AND after the server W2 wave emits real codes (legacy `AccessSchedule`/`StreamLimitExceeded` matches stay until W2 lands). Public surface: `ERROR_MESSAGES`, `ERROR_TITLES`, `GENERIC_ERROR_MESSAGE`, `errorCodeMessage`, `errorCodeTitle`, `isRegisteredErrorCode`, `PhlixErrorLocale`, `PhlixErrorCatalog` from the main index. `src/i18n/errors.test.ts` (49 pins) mirrors the locale-bundle law suite: registry parity per locale against `ERROR_CODES`, static-advice law (no `{token}`, no `|`), zero English leakage with an EMPTY bidirectional allow-list, ja CJK everywhere, the same diacriticals floors (measured margins es 97/fr 85/de 68/it 61/pt_BR 137), titles/generic completeness, and the full accessor degradation matrix including a sweep proving every registered code beats a fallback in all 7 locales; a real LoginForm mount test proves `{error:'Invalid credentials', code:'unauthorized'}` under `locale:'es'` renders the Spanish catalog sentence, not the server text. Era law: zero server change, zero contracts change, zero new subpath build entries. Suite 300 → 302 files, 6 072 → 6 131 passed / 10 skipped (+59, 0 regressions, 0 pins weakened); `dist/` changes by design — rebuilt committed, `dist:check` drift 0.

- **Six complete locale bundles make `@phlix/ui` the estate i18n source of truth (i18n build-out, owner directive 2026-09-23).** The config-time `PhlixAppConfig.messages` seam shipped untranslated; this adds translated catalogs for es / fr / de / it / pt_BR / ja — every group, every key of `DEFAULT_MESSAGES`, each bundle typed with `satisfies PhlixMessages` so a missing or extra key vs the English catalog is a COMPILE error. Plurals follow each language's CLDR cardinal category count (two segments for es/fr/de/it/pt_BR, ONE for ja — Japanese does not pluralize, so pipe-form English collapses to counter-phrases like `{count} 曲`), every `{placeholder}` is preserved verbatim, and the resolver/merge semantics are untouched: a bundle is just a full `PhlixMessages` fed to the existing seam, so `mergeMessages`/`createTranslator`/`useMessages` behavior is byte-identical for every existing consumer. Public surface: `ES_MESSAGES`…`JA_MESSAGES` + the `LOCALE_MESSAGES` registry + `PhlixLocaleCode` export from the main index (repo convention — tizen already imports the i18n types from `@phlix/ui`; no new subpath/build entry). `src/i18n/locales.test.ts` (50 pins) enforces at test time what the types cannot fully express: recursive key-set identity vs the catalog, `{x}`-token parity per message, the segment law (English pipes keep the locale's category count; NO `|` anywhere in ja; the one whitelisted additive plural is `player.subtitleDownloads`, where English hardcodes its plural while every call site already passes `count`), a bidirectionally-pinned English-echo allow-list per locale (brands, `you@example.com`, `SDH`, and true cognates like fr `Pagination` / de `Moderator` / it `Password` — stale allow-entries fail too), a ja CJK-rendering check with its exact Latin-only exception set, aggregate diacriticals floors for the five Latin-script bundles, `mergeMessages(bundle) === bundle` (zero English survives the merge), and separator-free `createTranslator` plural resolution at counts 0/1/2/7. README gains the structure / 7th-locale recipe / plural-segment-rules-table section. `dist/` changes by design (new shipped modules + widened barrel) — rebuilt committed. Era law: zero server change, zero contracts change, seam semantics untouched.

- **Overlay layers now remember their own opener, and an orphaned close parks on a defined anchor (W115 S534 — AD-9 ui focus-memory, the named adopt-leg of S526's tizen half).** Focus used to forget the stack: `useFocusTrap` kept a private per-instance `prevFocus` and restored it only if it was still in the DOM, so closing a layer whose opener had been removed leaked focus silently to `document.body`, and nothing anywhere could answer "how many layers are open?" or tear the memory down at once. Every active trap now holds ONE slot on a shared LIFO `LayerFocusStack` — push what held focus as the layer opened, pop it on close; closing an inner layer restores ITS opener while outer layers' slots survive (Modal-over-Sheet pinned both ways), and an active trap that unmounts mid-flight releases exactly its own slot non-LIFO — pop & stash & re-push through the stack's four ops, never stealing focus from the layer above. When the LAST layer closes over an orphaned opener, focus parks on a defined spatial anchor instead of dangling: an explicit `[data-focus-anchor]` (hosts mark their top bar) wins, else the first live entry of the focusable registry — both TV-opt-in, so a desktop app that registers neither sees the pre-S534 posture byte-identically, including the silent null/body-opener paths (single-layer live-opener restore is pre-existing behavior, NOT re-built — the shipped delta is the stack accounting, the orphan park and the teardown clear, and this bullet says so). The mechanism is synchronous park-on-close; the study's 50 ms orphan-SWEEP alternative was deliberately not shipped — one mechanism, never both. `src/composables/layerFocusStack.ts` is a shape-match port of the tizen client's `createLayerFocusStack` (identical `push/pop/depth/clear`), not an import: the dependency direction is phlix-tizen-client consuming `@phlix/ui`, and a source-lock grep pin holds zero `phlix-tizen-client` imports in this repo — convergence into one shared source is a future pin-batch discussion, documented, never pre-built; the module is pure bookkeeping (a DOM-free unit spec proves the four ops over opaque string nodes; a second pin greps the source for engine tells — no lifecycle, keys, geometry or DOM access — so it can never rot into a second spatial-nav engine). `useSpatialNav`'s handle gains the additive layer seam `layerDepth()` / `clearLayerFocus()` (passthrough only — gating, the `[data-focus-trap]`-yield and the typing-target-yield are untouched and still pinned by their existing suites, zero pins weakened). Honest remainder: the AD-9/TN-7 legs NOT adopted here are the `PhlixApp.vue` `<KeepAlive>` route-caching increment and per-route `lastFocusedKeyByPath` screen memory — this block is the overlay/modal layer leg the owner directive named; the tizen consumer's benefit of the shared shape rides the next `@phlix/ui` pin-batch (ui-only PR, S532/S533 precedent). Era law: zero new server routes, zero contracts change, zero server delta. Covered by 21 new pins (pure stack ops incl. null-slot order integrity and instance isolation; no-fork + no-engine + shape source-locks; push/pop depth; Modal-over-Sheet per-layer restore + outer survival; non-LIFO unmount; anchor park; registry-fallback park; no-anchor desktop silence; live-opener-wins-over-anchor; clear-then-close safety; spatial-handle layer-seam passthrough); suite 298 → 299 files, 6 001 → 6 022 passed / 10 skipped (+21, 0 regressions, 0 pins weakened); drift-proven red: disabling the park call reddens exactly the three park tests. `dist/` changes by design (two shipped modules changed) — rebuilt committed, `dist:check` drift 0.

- **A dev-only TN-6 subtitle-budget bench now rides the visual-harness rails (W114 S533).** The open question it exists to answer: does an entry-level Tizen TV have frame budget for custom ASS subtitle rendering, or should subtitles stay on the native WebVTT path — decided by MEASUREMENT, before anyone commits to (or rejects) the AD-7 renderer. New `src/dev/visual/subsBench.html` + `subsBench.ts` (a pure, side-effect-free core: capability probe, mode planner, frame-time statistics, and hand-rolled ASS/VTT parsing — including the karaoke `\k` sweep decomposition and the &HAABBGGRR colour-order trap) + `subsBench-boot.ts` (dev-shell mount, kept out of the core so the worker can import the parser without touching CSS-injection) + `SubsBenchHarness.vue`: a scripted run (default 60 s, seek stress every 2 s) with a rAF frame-time sampler reporting avg fps / p99 frame-ms / PerformanceObserver long-task count across three paint paths side-by-side — main-thread parse+paint (the worst case priced), worker-parse + main-paint, and the plain-UA-captions baseline floor. Shipped fixtures: an ASS clip with an 8-style 16→96 px font table, multilingual glyph runs (EN/JA/RU/AR/HI/ZH/KO), karaoke sweeps and multi-line blocks, plus a mirrored WebVTT file for the UA leg. `run-subsbench.mjs` boots the SAME Vite dev toolchain and a headless Chromium and stamps machine identity into the JSON artifact; modes the platform cannot service are enumerated skipped-WITH-REASON and never produce numbers — and the honesty clauses are structural: the Tizen 6.5-emulator and entry-level real-TV legs are reported UNREACHABLE at this venue (no SDK, no paired TV), so the numbers this merge publishes are LOCAL-CHROMIUM, labelled as such in the artifact, never dressed up; the raw-ASS delivery gap (the server normalizes sidecars to WebVTT, so production never ships raw ASS to a renderer) is documented in the artifact, not papered over. Zero production coupling is PINNED, not promised: `src/dev/**` is outside the lib build, and a gate test greps EVERY committed `dist/` bundle for any `subsBench` string (zero hits) and `package.json` for `jassub` (absent) — this step ships a measuring instrument, not a renderer. The S241 base-awareness gate gains two classified exceptions (a local fixture byte-stream and a `blob:` caption track in a dev-only component — neither is ever a media-server payload). Vitest scope follows the survey-b honesty caveat (stated in the test header): capability-gate + fallback logic + parse/statistics math only — jsdom has no compositor or media pipeline, so frame numbers come only from the browser driver. Covered by 19 new pins; suite 297 → 298 files, 5 982 → 6 001 passed / 10 skipped (+19, 0 regressions, 0 pins weakened). `dist/` is byte-identical — verified by building over the change: 0 drift entries.

- **The Connect screen can now offer host-discovered servers (W114 S532).** Typing a LAN address with a TV remote is the worst possible input method, and the estate already has a LAN discovery engine — on `phlix-tizen-client`, where it currently has zero production callers. This step wires the CLIENT SEAM, not a second engine: `PhlixAppConfig` gains an optional `connectScan?: () => Promise<ConnectCandidate[]>` (new exported `ConnectCandidate = {url, label?}` type), and the Connect page renders a "Scan your network" button + the bounded candidate list it yields, natively focusable rows so the D-pad reaches them. A picked row prefills the address field and commits through the page's EXISTING `commitWithGuards` gauntlet — the plaintext-http public-host warning and the one-time new-origin token-send confirm stay fully in force, and `withScheme` still owns the scheme law, so a hostile or malformed candidate can no more silently become the API base than a typed one can. Honesty boundaries: no candidate is re-probed (the host already verified what it yielded — probe construction lives ONLY in `useConnectionStore`), nothing is rendered the host didn't yield (no ui-side subnet guessing), and with no scanner injected the entire affordance is absent from the DOM (zero dead UI, pinned). This PR is ui-only BY DESIGN: the tizen shell rides the seam in the next pin-batch (its `lanDiscovery` module exposes `sameSubnetCandidates`/`discoverServers`/`buildConnectSuggestions` — the `connectScan` closure will adapt them; documented here because the seam is dead surface until then). A source-lock test pins the absence of any second engine (`/health` construction, discovery-engine names) inside the page. Five new i18n strings join the `connect` group. Era law honored: zero new server routes, zero contracts change. Covered by nine new pins (hidden-without-scanner, as-is rendering, guarded commit with a throwing fetch double proving no re-probe, already-confirmed straight-through, reject → honest note, empty → empty note, malformed candidate refused at the boundary, source-lock, token self-pin); suite 297 files, 5 973 → 5 982 passed / 10 skipped (+9, 0 regressions, 0 pins weakened). `dist/` changes by design — and churns WIDELY this time for a real reason: the new i18n strings move the shared `useMessages` chunk, whose content hash every page chunk embeds, cascading renames across nearly the whole graph (216 new / 216 deleted / 9 modified); a second consecutive build reproduces the identical name set, confirming determinism.

- **The hub media-requests portal gained a debounced search/filter and a late-arrival-safe reload (W111 S519).** `RequestsPage` already rendered the full `GET /api/v1/me/requests` list but gave no way to narrow a long list, and every create/delete reload raced whatever load was already in flight. It now has a 500 ms debounced title search plus an immediate status filter (Pending/Approved/Available/Rejected), with a distinct "No matching requests" empty state and a one-click Clear when nothing matches. The debounce is a new shared `src/utils/debounce.ts` primitive — one timer source whose re-invoke supersedes any pending fire — so a keystroke burst recomputes the list exactly once with the latest query rather than once per character (the AD-16 "no boolean soup/flicker" contract). The util ships as NEW shared surface, not a replacement sweep: `RequestsPage` itself had no timer pair to replace, and the repo's existing hand-rolled `clearTimeout`/`setTimeout` sites (`SearchPage.vue`, `FilterBar.vue`) are untouched by this step — migrating them onto the shared primitive is a queued follow-up. The list reload is now sequence-guarded with the estate's established drop-late-arrivals discipline: a reload superseded by a newer one discards its result instead of clobbering the fresher list or re-flashing rows the user just acted on. Honest re-derivation of AD-17: there is **no `caches.open`/CacheStorage artwork layer in `@phlix/ui`** — the browser-image-cache premise does not exist in live source — and the one bounded ui cache, `useMediaItemCache` (the perf-claim-5 SWR JSON cache), is deliberately untouched; no request-list store was fabricated. Filtering is purely client-side over the already-loaded rows, so **no new server surface** and no API contract change. Covered by new `debounce` util tests (trailing collapse, cancel/flush, pending flag, argument pass-through, the 400–600 ms window) and six new `RequestsPage` pins (toolbar appears only when populated; applies after the window and not on keystroke; burst collapses to the last query; status filters immediately; no-matches + clear; and a resolve-ordering test proving a late superseded reload is dropped). Suite 294 → 295 files, 5 936 → 5 954 passed / 10 skipped (+18, 0 regressions, 0 pins weakened); `dist/` changes by design (shipped component + new util changed).

- **A constrained device downlink now caps the initial transcode quality rung (W110 S514).** A transcode ladder always opened at the same profile regardless of the bandwidth the device actually reported, so a phone on a thin link still asked the server for the full 1080p rung before hls.js could notice and thrash down. The player now reads `navigator.connection.downlinkMax` at the transcode boundary and derives a conservative target (0.7× to leave headroom, since `downlinkMax` is an optimistic ceiling), then maps that to an **existing** server `?profile=` hint — lowering the rung only (`mobile-high` when it fits 4M, `mobile-low` as the floor). It NEVER raises above the `web` baseline a browser already gets. When the Network Information API is absent, reports `Infinity` (unbounded — the honest reading on many wireless links), NaN, or a non-positive value, no profile hint is sent at all and the request is byte-identical to today. Pure, DOM-free cap math lives in the transcode helpers; the browser read is isolated to the component boundary. No server change — this consumes only the `?profile=` parameter the transcode start already honored. Covered by pure tests (conservatism factor, every absent/unbounded path → no hint, mobile-high and mobile-low derivations, never-above-baseline) plus component pins asserting the exact `tc.start()` argument through a stubbed `navigator.connection`. `dist/` changes by design (shipped component changed).

- **The player now escalates an unplayable source by re-asking playback-info with the right constraints (W110 S513).** A fatal decode error, or a source that starts but never renders a frame, already fell through to the transcode rung — but the retry carried no information about *why*, so the server could hand back a ladder that still offered the exact thing the browser had just proven it cannot play. This is the caller-side half of the server's constraint inputs: at both decision points, before the give-up surface, the player now re-requests `playback-info` with `forceTranscode=1`, and additionally `excludeHevc=1` when the source's own video stream is HEVC-family (classified through the same codec table the direct-play guard uses, never a second spelling list). A started-but-stalled source gets a single 30-second watchdog — deliberately the estate's existing first-byte budget, not a new number — armed once on `play` and cancelled the moment frames advance, so a healthy start never escalates and repeated play events never stack. When the API is absent (a first load) neither param is sent and the request is byte-identical to before. Covered by pure tests on the param derivation and the stall predicate, and by component tests with a fetch double asserting the exact retry URLs on the HEVC and non-HEVC fatal paths, the once-per-media guard, the escalation after a real stall, and that advancing playback fires nothing. No server change. `dist/` changes by design (shipped component changed).

- **Hidden chrome is now focus-contained (W110 S512).** Auto-hiding the OSD only faded it — the metadata row, the center play/pause and the whole bottom control cluster were still tab-focusable, so a D-pad could park on a control the viewer could not see. Native `inert` would fix it but requires Chrome 102, above this app's build floor, so the portable mechanism is blur + `tabindex="-1"`: when the chrome hides, the focused control blurs (focus moves OUT, not merely covered) and every focusable inside the surviving clusters is pulled out of the tab order, with each element's own `tabindex` snapshotted and restored verbatim the moment the chrome shows again. The ±10s skip targets need no handling here — they have been DOM-removed while hidden since the center transport landed; this step simply extends that same hidden-≠-focusable law to the rest of the chrome. Covered by new targeted tests driving the real auto-hide timer and jsdom's real focus model: a focused control is blurred on hide, the hidden controls carry `tabindex="-1"`, and both attributes are gone once any reveal gesture brings the chrome back. `dist/` changes by design (shipped component changed).

- **The final seconds of playback are now reported on quit / teardown (W109 S506).** The cross-device resume writer checkpointed position on a 15s throttle and on each play/pause flip, but nothing flushed on unmount — so everything watched between the last checkpoint and closing the player (route-leave, or the mini-player's close) never reached the server, and the next device resumed up to fifteen seconds behind. `useResumeReporter` now retains the freshest in-band position on every reportable tick and exposes `reportFinal()`, a forced flush that posts that position regardless of the throttle. It prefers the live store position and falls back to the retained checkpoint, so it is correct whether it runs before or after the store is cleared; like `finish()` it never spins up a session just to report the tail, so a title under the 30s resume floor is left untouched. The full `Player` flushes on `onBeforeUnmount`, and the persistent `MiniPlayer` flushes both on its close (before `closePlayer()` nulls the item) and on component teardown. The client 30s min-playtime gate is unchanged — this only closes the client tail gap. Closing live-stream ids on unmount is n/a here: the resume session is idempotent per device id, not per stream, so there is no per-stream session to tear down and the final-position flush is the whole quit contract. Covered by new targeted tests pinning that the flush sends the last retained position after the store is nulled (not a zeroed read), sends the live sub-throttle tail when the store is intact, no-ops without a session or when logged out, swallows failures, fires from both the full player and the dock on unmount / quit, and — order-sensitively — that the dock's close reads a still-live store at the instant it flushes, so reordering the flush after `closePlayer()` reddens the test. `dist/` changes by design (shipped composable changed).

- **Center-cluster ±10s transport (W109 S505).** The play/pause button at the middle of the player stood alone: skipping ten seconds was reachable only from the keyboard J/L chord or a remote, never from the pointer or a D-pad focus stop (the bottom row's skip-back/forward icons navigate EPISODES, not the timeline). The center cluster is now a three-button transport — replay-10 · play/pause · forward-10 — each skip invoking the same `seekBy` action the shortcuts already drive, so the position is clamped to the media exactly as the scrubber and keyboard already are. The two skip targets render only while the chrome is up, so a hidden overlay leaves them out of the DOM entirely rather than focusable-but-invisible (AD-11: hidden ≠ focusable, so a D-pad can never strand on them), and each carries a numeric `10` badge over a rotate glyph. Two new English strings, `player.seekBackward` / `player.seekForward`, back the accessible labels. Covered by new targeted tests pinning that both buttons render when the chrome is shown, move the playhead by exactly ∓10s (and clamp at the boundaries), reuse the shared `seekBy` action, and disappear when the chrome auto-hides. `dist/` changes by design (shipped component changed).

- **"Who's watching?" — the self-service profile store, gate and manager (S82).** The ten AuthMiddleware profile routes S81 shipped finally have a client half: `useProfileStore` owns the list (`GET /api/v1/profiles`, active-first, hydrated rows), the localStorage active-id MIRROR (`phlix.active_profile` — a first-paint hint only; the server's `is_active` row is adopted on every load) and the switch handshake, which goes to the REAL `POST /api/v1/profiles/{id}/switch` and adopts everything that response IS — re-minted access/refresh pair through `useAuthStore.setTokens`, the `user` payload, the new active id — so no read ever rides a token from the previous profile (a local-only flip would have rendered the old profile's favorites under the new name; AC-3's "no local-only lie"). `PhlixApp` mounts the `WhoIsWatching` overlay (role=dialog, tile grid, its own loading/error/Retry states) whenever `gateOpen` holds: loaded + MORE THAN ONE profile + no choice yet — single-profile accounts land directly in the app, and a failed PASSIVE read opens no gate nobody can pass — but an explicit "Switch Profile" click ARMS the surface (`arming`), so its loading/error + Retry panes stay reachable rather than the click being dead — and re-activating the already-active row closes the gate without a round-trip. Cache honesty is one signal: `epoch` bumps only on a real id→different-id swap, and `useUserItemDataStore` (per-item favorite/watched flags), the Browse rails (favorites / Next Up / Recommended / resume — NOT the global Most-Watched leaderboard) and `WatchHistoryPage` all clear and re-read on it. UserMenu gains "Switch Profile" (re-arms the gate — and user-initiatively retries a failed boot read) and "Manage Profiles" (new `/app/profiles` route on `ProfilesPage`: inline rename via PUT, create via POST, delete via DELETE with the server's 409 last-profile refusal surfaced verbatim and the client guard disabling delete at one row). `AdminUsersApi` grows the five self-service methods beside the admin ones (the route gate now drives 31 distinct urls for that class); the vendored server manifest is untouched — this step adds NO new server routes. 40+ new tests across store/both screens/shell/menu, each AC pinned at its layer; mutation-proved: deleting the `>1` clause reddens exactly the named single-profile tests. Suite 290 → 293 files; CI-verbatim suite at the #378 merge: 5 872 passed / 10 skipped; `dist/` changes by design (new shipped components).

- **The update card now TRIGGERS a check — S273 ui half.** phlix-server shipped `POST /api/v1/admin/updates/check` (`1e14b539`); the honest S78-era read-only button ("Check update status", card prose literally said "there is no endpoint that forces one") became a lie at that server commit, so the label, the prose and the wiring move together: `AdminUpdatesApi.check()` posts the trigger (empty body, abort-signal forwarding, the 202's persisted-at-response status parsed through the SAME `parseCoreUpdateStatus` as the status read) and `TasksPage` renders from it; feedback lines now begin "Check dispatched." Four API tests + three rewritten page tests pin the trigger path (constant AND literal), the no-extra-GET shape, and the failure-reason passthrough; the route gate drives both methods and pins the distinct set `GET …/status` + `POST …/check` against the vendored 401-tuple manifest. `dist/` churns by design this time — the shipped card copy changed.

- **All NINE sibling `disabled` guards are now pinned by raw `dispatchEvent` tests, each mutation-proved (S320).** `MusicPager.test.ts`'s post-mortem showed Vue Test Utils' `trigger()` short-circuits on disabled controls, so trigger-driven "disabled" tests are green no-ops — the trap that let eight guards be deleted with the suite green. The follow-up it named said SEVEN sites; the live count at this tip is NINE `props.disabled`-style early-returns across six files — the two extra keydown-path guards (`Select.openList` beside `Select.onTriggerKeydown`; `Combobox.openList` beside `Combobox.onKeydown`) pin real behaviour too and are pinned equally: `ThumbRating.onUp`, `ThumbRating.onDown`, `Switch.toggle`, `Chip.onMain`, `Select.openList`, `Select.onTriggerKeydown`, `Combobox.openList`, `Combobox.onKeydown`, `Tabs.select`. The vacuous disabled tests in `ThumbRating.test.ts`, `Tabs.test.ts` and `Select.test.ts` were rewritten to raw dispatch with an ENABLED raw-dispatch control test per file proving the route is not inert; new `Chip.test.ts`, `Switch.test.ts` and `Combobox.test.ts` cover the components that had none (no test file existed for them at all). The keydown guards are pinned independently of the open-list guards by reaching the handler with the list ALREADY open on a component disabled afterwards — the only route that sees deletion of one guard without the other masking it — and also pin that a disabled control must not even call `preventDefault()`. `Switch`'s `<label>` carries the same handler with NO native `disabled` attribute, so there the JS guard is the only barrier — pinned. Every guard: delete → named test reddens → restore → md5-identical → re-green. Suite 286 → 289 files, 5 808 passed / 10 skipped (+18 tests, 0 regressions); `dist/` byte-identical (test-only + comment-only changes).

### Changed

- **`@phlix/contracts` re-pinned `#v0.5.1` → `#v0.5.2` (estate contracts v0.5.2 re-pin cascade) — and the version-FIELD SKEW ENDS: the lock entry moves `0.4.7` → `0.5.2` honestly.** The v0.5.2 annotated tag (object `9676874e`, peels to `7afb6a91`) is a PIN-CARRIER release for this repo: everything ui actually consumes measures byte-identical across the bump — `dist/error-codes.json` md5 `b919685d…` UNCHANGED (202 codes / 37 domains, census re-run on the fresh `npm ci` tree), the main entry `dist/phlix-contracts.js` byte-identical (md5 `6cd0a00e…`), and `dist/server-route-manifest.json` byte-identical at md5 `06ce7ec9…` — still exactly the vendored `src/api/test/server-route-manifest.json` copy, so the `routeGateServer.ts` vendoring claim survives the bump unchanged (vendoring persists for the same reason: the contracts `exports` map still ships only `.` + `./package.json`). The tag's `dist/errors.d.ts` churns by +858/−617 lines but is DOCBLOCK-ONLY: comment-stripped, v0.5.1's and v0.5.2's declarations are string-identical (machine-proven) — the contracts errors-coordinate re-sweep (#79–#81: ticket CI gates, `verify:cites` tripwire, `server.not_found` reuse-target registration) simply rides the declarations, consuming nothing new at this boundary. THE KEY EVENT: the estate release-identity precedent had frozen contracts' package.json `version` FIELD at `0.4.7` through BOTH the v0.5.0 and v0.5.1 tag trees (tag NAME = release identity; ui mirrored it at v0.99.5); v0.5.2 finally advances the field to `0.5.2`, so the lock entry records `"version": "0.5.2"` for the first time in the aligned era, and the #421-era SKEW NOTE at the guard is REWRITTEN as a SKEW LINEAGE NOTE — history kept (v0.4.7 → v0.5.0/v0.5.1 skew era → v0.5.2 alignment), with the equality-era rule named: the new lock `version`-field assertion (`PINNED_CONTRACTS_VERSION = '0.5.2'`) holds ONLY while field and tag agree and must be relaxed again if a future release re-skews — the tag-pin and peel anchors never depend on it. `contractsPin.test.ts` advances honestly with the pin (tag regex → `#v0.5.2`, `PINNED_CONTRACTS_PEEL` → `7afb6a91…`, titles, plus the new fourth assertion); the pin-naming cites re-truth `v0.5.1` → `v0.5.2` in `errors.test.ts` (registry-count pin stays 202 — the census is the proof), `LoginForm.test.ts`, `routeGateServer.ts` (and its committed `dist/api/test/routeGateServer.d.ts` — the ONLY dist delta in this PR, a comment string), and the README registry line. Lock diff minimal (3+/3−: spec echo + `version` field + `resolved` peel). Suite 302 files, 6 143 passed / 10 skipped (6 153 total; +1 = the new equality-era assertion, zero regressions, zero pins weakened); `dist:check` drift 0 after committing the rebuilt tree; build idempotence proven (863-file md5 census identical across consecutive rebuilds); npm 11.19.0 CI-faithful install, `npm ci --allow-git=all`.

- **`@phlix/contracts` re-pinned `#v0.5.0` → `#v0.5.1` (estate contracts v0.5.1 re-pin cascade — the Wave-1b registry expansion, consumed).** The v0.5.1 annotated tag (peels to `e3c14f07`) expands the error-code registry 147 → 202 codes across six NEW domains (`claim`, `access`, `identity`, `provider`, `oauth`, `ldap`) plus additions to existing ones (auth +5, hub +2, server +1, stream +1, profile +1, alexa +14; 31 → 37 `ERROR_DOMAINS`) — PURELY ADDITIVELY: the installed `dist/error-codes.json` diff vs v0.5.0 measures 55 added / 0 removed / 0 changed and the main-entry `ERROR_CODES` measures 202. Lock diff stays minimal (spec echo + `resolved` peel move); the version FIELD keeps `0.4.7` — the deliberate skew documented by the #421 SKEW NOTE still applies (the v0.5.1 tag tree's own package.json reads `0.4.7`; the tag NAME is the release identity). Guards advance honestly with the pin: `contractsPin.test.ts` tag regex + `PINNED_CONTRACTS_PEEL` → `e3c14f07…` + titles; `errors.test.ts`'s registry-count pin 147 → 202; the `routeGateServer.ts` vendoring cite is re-truthed and its claim HOLDS (v0.5.1's `server-route-manifest.json` measures byte-identical — md5 `06ce7ec9…` — to v0.5.0's and the vendored copy), so the vendoring persists for the same surviving reason (the contracts `exports` map still blocks the JSON subpath import). THE REAL DELTA IS i18n: every catalog in `src/i18n/errors.ts` is a total `Record<ErrorCode, string>`, so the expansion is a COMPILE error until every locale answers — now all seven locales carry all 202 codes (385 new sentences, authored to the W4 content laws: short honest user-facing sentences from the registry's per-code emit-site semantics, no `{placeholder}`/no `|` anywhere, zero English leakage both directions against the empty allow-list, ja fully CJK, diacriticals floors held not relaxed with widened margins es 135 / fr 128 / de 94 / it 95 / pt_BR 189 — measured, floors 50/50/50/15/50). README's registry citation line re-truthed (202 codes / 37 domains / `v0.5.1`). Suite 302 files, 6 138 passed / 10 skipped (6 148 total — delta 0: the parity suite derives from `ERROR_CODES`, so it covers 202 codes × 7 locales without any new test body); `dist/` churns by design (widened error catalog + guard comments in declarations) — rebuilt committed, `dist:check` drift 0; build idempotence proven.

- **`@phlix/contracts` re-pinned `#v0.4.7` → `#v0.5.0` (estate contracts v0.5.0 re-pin cascade).** The v0.5.0 annotated tag (peels to `8ef65d3`) lands the ERROR_CODE registry — `src/errors.ts` (147 codes / 31 domains) and the `dist/error-codes.json` emit — PURELY ADDITIVELY over v0.4.7: every pre-existing export is byte-unchanged, and this repo consumed zero of the new surface, so the bump is pin-only with zero source adaptation (typecheck/lint/build/test all green on the new types with no edits outside the guard). `contractsPin.test.ts` advances honestly with the pin: the tag regex, the `PINNED_CONTRACTS_PEEL` integrity anchor (`625a5625…` → `8ef65d30…`) and the test titles. VERSION-FIELD SKEW (deliberate, documented at the guard): the v0.5.0 tag tree's package.json `version` FIELD still reads `0.4.7` — the tag NAME is the release identity, mirroring ui's own v0.99.5 precedent — so the lock entry keeps `"version": "0.4.7"` and the guard anchors only tag name + resolved peel, never the version field. `routeGateServer.ts`'s vendoring cite is re-truthed for the new tag: the claim it made ("v0.4.7 PREDATES the provenance regen") does not survive the bump — v0.5.0's `dist/server-route-manifest.json` measures byte-identical to the vendored copy (md5 `06ce7ec9…`) — but the vendoring itself stays sanctioned because the contracts `exports` map (only `.` + `./package.json` subpaths) still blocks the JSON subpath import the drop-the-vendored-copy plan needs; the rewrite names that as the real remaining gate. `dist/` churns in exactly the cite's own `routeGateServer.d.ts` (comment embedded in declarations); contracts' new types compile to zero runtime delta. Suite 300 files, 6 072 passed / 10 skipped (6 082 total, unchanged); `dist:check` drift 0; build idempotence proven (rebuild → clean worktree).

- **Admin JS is now genuinely droppable for consumers that never mount it (W113 S528).** `createPhlixApp.ts` statically imported `adminPageLabel` from the admin barrel for one route-title lookup, so the whole page-descriptor registry stayed live in the graph of every consumer — the lazy `() => import('../pages/admin/X.vue')` closures remained reachable and bundlers emitted every admin page chunk no matter what (the S517 tizen receipt measured exactly this leak: the TV-admin ternary could drop the layout chunk while 19 page chunks survived "because the shell statically imports the admin registry for label lookup"). The fix is a new side-effect-free seam, `src/app/admin-registry.ts` (`setAdminLabelResolver` / `adminLabelFor`): the shell imports ONLY that, and `buildAdminRoutes` self-registers the label resolver as its first line — a host that mounts admin routes gets identical `Admin · <label>` titles with ZERO consumer API change, while a host that mounts none can drop the barrel entirely. `admin.ts`'s module top level is now pure literals as well (the eager `Object.fromEntries` label map moved into a lazy memo inside `adminPageLabel`), because a module-scope function call is what made even a conservative bundler retain the module. Measured on the REAL tizen app built with its own vite 7.3.6/classic-rollup toolchain with `VITE_PHLIX_TV_ADMIN` unset: **19 admin page chunks / 651,017 B → 0 / 0 B; total app JS 68 chunks / 2,572,509 B → 35 chunks / 1,828,937 B — −743,572 B (−28.9% of built app JS)**; with the flag ON the same build still emits all 19 page chunks + layout, so admin-bearing consumers' byte-shape is unchanged. Honest limits: (a) the CSS half does NOT ship — `cssCodeSplit:false` is a UI-3.3 requirement (lib-mode async-chunk CSS emits as `/* empty css */` and is never runtime-injected, so split admin CSS would be inert), and carving an `admin.css` out would break the single-stylesheet contract for consumers pinned at `#v0.99.4`; measured by build subtraction, admin CSS is 98,192 B of the 434,329 B `style.css` (22.6%), recorded as a known limitation for a consumer-bump era. (b) The repo's own vite 8.2.0/rolldown synthetic harness shows the same drop (−399,155 B) but keeps 7 fully unreferenced page chunks — rolldown conservatism, disclosed. No public API moved: `adminPageLabel` remains exported from `./admin`; the registry seam is internal, and `index.ts` exports are unchanged. Covered by new `adminLazy.test.ts` (unregistered seam → null title, registration through the REAL `buildAdminRoutes()` path, hub-builder delegation, source lock forbidding a static `./admin` import in the shell) plus the existing page-title admin cases now registering via the real mount path; suite 296 → 297 files, 5,967 → 5,973 passed / 10 skipped, 0 regressions. `dist/` changes by design (the shipped shell changed) — including a byte-size-identical `style.css` reorder (434,329 B both sides, one rule's position moved) and patch-level `.d.ts` regeneration drift from the fresh npm-ci toolchain rebuilding an older-toolchain-committed dist; all committed as-built, `dist:check` drift 0.

- **The two named SearchPage/FilterBar debounced-search pairs now ride the shared `debounce` primitive (W112 S524 — the migration S519's changelog bullet explicitly queued).** `SearchPage.vue` (fixed 300 ms window, shared by `@input` and `@submit`) and `FilterBar.vue` (window read from the public `searchDebounce` prop — default 250 ms — at every keystroke, with `onBeforeUnmount` cancelling the pending fire) each carried their own `let timer` + `clearTimeout`/`setTimeout` pair; both pairs are gone, replaced by `src/utils/debounce.ts`'s one-timer supersede trailing wrapper with the call-site's own ms passed explicitly. Zero behavior drift was the law: each site's current timing semantics were captured as pins FIRST at the pre-migration tip (exact 299/300 and 249/250 boundaries, burst-collapse to the latest query, cross-trigger re-arm of the same timer, FilterBar's unmount cancel and the re-read-on-every-input prop delay, and the honest status-quo facts — SearchPage has no unmount cancel at all, its pending fire surviving teardown is pinned as-is rather than "fixed", and FilterBar's clear button never cancelled the in-flight timer so a `change` re-emit on the cleared value is pinned too), then the migration commit re-ran the identical pins green. FilterBar's per-input prop read is preserved by re-arming the wrapper inside `onSearchInput` (cancel previous → build with current `props.searchDebounce` → call), because a primitive configured once at setup would have frozen the delay — drift. The primitive needed no capability additions; `src/utils/debounce.ts` is untouched. Scope honesty: `debounce.ts`'s own docblock names further timer sites (the media/preferences stores, the URL-sync composable) — this step migrated exactly the two pairs the S519 follow-up named, and source-scan tests now lock `setTimeout(`/`clearTimeout(` out of both migrated components. Covered by the new pin suite + 4 source locks (9 + 4 tests); suite 295 → 296 files, 5 954 → 5 967 passed / 10 skipped (+13, 0 regressions, 0 pins weakened). `dist/` changes by design (both shipped components changed).

- **Route-manifest CONTENT regen + currency re-pin — cs#47 currency cascade (2026-09-16), NOT pure.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #34 against the current phlix-server master tip — the quick-connect pairing / client-heartbeat server additions). This span is a genuine route change, not a provenance-only re-pin: the union rises from 404 to 410 tuples (the Application guard count rises 367 → 373 with the six new quick-connect/telemetry tuples; WebPortal 48 and shared 11 hold), so route-content bytes move alongside the two provenance lines. Currency pins advance in the same commit — the full-file md5, full serverSha and contracts-tip cite in `routeGate.api.test.ts`, and, as at the last content regen, the tuple-count pin, the provenance-total pin, the union source split and the vendoring cite in `routeGate.harness.test.ts`. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.7` pin tracks tags and this wave is untagged. No new token home in this repo — the cs#47 token stays in its single contracts code home and this repo's existing cs#46 code-line constant holds. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0).

- **Subtitle lift is now measured, not a fixed 88px (W109 S504).** While the player chrome is up, the custom caption overlay raised itself by a hard-coded `88px` above its resting line — the wrong number for every control cluster taller than a bare scrubber + button row, so the MarkerTimeline and quality rows could sit under the captions. The Player now hands the control cluster (scrubber + marker timeline + button row) to `CaptionOverlay` as `controlsRoot`; the overlay measures that element's real height with a `ResizeObserver` and binds it to a `--phlix-sub-offset` custom property, and the lifted state translates up by exactly that amount (a `0px` default keeps captions seated until a height is measured). The offset collapses to `0` the moment the chrome hides, so the captions reset; the observer is disconnected on unmount. `ResizeObserver` only — no `MutationObserver` — and it is guarded so jsdom/SSR (where it is undefined) degrades to a seated overlay rather than throwing, which is the custom render path's behaviour regardless (native `::cue` never mounts this component). Covered by new targeted tests pinning measure→offset, the re-measure on a resize, the reset when hidden, the no-`ResizeObserver` fallback and the unmount disconnect. `dist/` changes by design (shipped component changed).

- **Route-manifest provenance re-pin — cs#46 currency cascade (2026-09-15), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #33 against the current phlix-server master tip — route purity re-proven in-lane before vendoring: the vendored-vs-canonical diff is EXACTLY the two provenance lines, `serverSha` + `generatedAt`, every route-array byte unchanged, both sides measuring 404 tuples); the currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cite advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.7` pin tracks tags and this wave is untagged. The wave token gains a code home in this repo: `routeGate.api.test.ts` exports it as a code-line constant adjacent to the pins (comment-strip-proof; adds no test case). Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0).

- **`routeGateServer.ts` stale contracts-pin cite + code-resident survival pin (S497, doc-only).** The vendoring rationale still cited `@phlix/contracts#v0.4.5` while the install pin moved to the `#v0.4.7` tag (the `contractsPin.test.ts` truth since the W85 retag); the sentence's claim survives the bump unchanged — `v0.4.7` still predates the untagged provenance regens, exactly why the copy stays vendored — so only the tag literal advances, in `src/api/test/routeGateServer.ts` and its committed `dist/api/test/routeGateServer.d.ts`. The wave's survival token lands on a CODE line (`export const S497_DOC_PIN_TOKEN = 'S497DOCPINX9P7';`) because the merge-gate comment-strip defense removes prose tokens from `--token-in` sources before the survival check. Test-only helper: nothing shipped changes; `dist/` churns in exactly this helper's own `.d.ts` (cite line + the new declaration), rebuilt and `dist:check`-proven at drift 0.

- **`contractsPin.test.ts` prose + identifier honesty pass — S491, XS hygiene.** The W85 retag hardening re-pinned this guard's VALUES to the `#v0.4.7` contracts tag truthfully, but left two prose-layer artifacts: the JSDoc still cited the PREVIOUS tag's peel (a stale shortened hash) as the pinned tag's resolution — provably false against the lockfile's single `0.4.7` resolution — and the constant holding the correct peel was still named after the old tag, a misnomer that would mislead the next reader (the retag engine keys on VALUES, not identifiers, so renaming is safe). The docblock now names the constant instead of quoting a hash — provenance-neutral, so it cannot go stale on a future re-pin; the constant is the single source of the peel literal. The constant is renamed to `PINNED_CONTRACTS_PEEL` at both sites (declaration and the template-literal reference); the peel VALUE stays byte-identical and every assertion stays behavior-equivalent — the tag-pin regex, the exactly-once resolution check and the fail-fast throw are untouched. Test-only change; `dist/` byte-identical after a full rebuild (drift 0); suite counts unchanged (this file 3/3, full-suite before/after tallies in the lane journal).

- **Route-manifest provenance re-pin — cs#45 currency cascade (2026-09-13), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #32 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is a bundle-only web-ui re-pin, an empty diff under `include/` and `src/` and on the two route-bearing guard-constant files, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 404 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cite advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.7` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#44 currency cascade (2026-09-13), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #31 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is a bundle-only web-ui re-pin, an empty diff under `include/` and `src/` and on the two route-bearing guard-constant files, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 404 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cite advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Music lookup migrates to the S240 query-param rails + route-manifest CONTENT regen — cs#43 currency cascade (2026-09-12).** S240 promoted the by-name music lookups to query-param GET rails (`GET /api/v1/music/artist?name=` and `GET /api/v1/music/album?name=[&artist=]`, additive — the legacy `/music/artists/{mbid}` and `/music/albums/{mbid}` paths are still served). `ApiClient#getArtist` and `#getAlbum` in `src/api/client.ts` now build the query rail (the album artist disambiguator rides `&artist=`), with their JSDoc route cites moved to match; the four page/API fixtures that pinned the old path form (`MusicArtistPage.test.ts`, `MusicAlbumPage.test.ts`, `MusicLibraryPage.test.ts`, `client.test.ts`) are re-pinned to the new form, and the route-gate EXPECTED set moves its two music detail tuples to the query rails (list tuples unchanged). This is the AC: the route gate and the URL-pinning fixtures RED if the client migration is reverted (proven by a temporary revert — 36 named tests redden across the four files, then re-green on restore). Because S240 ADDED routes, the vendored `src/api/test/server-route-manifest.json` is a CONTENT regen (not a pure re-pin): re-vendored byte-identical from `@phlix/contracts` master (untagged regen #30 against the current phlix-server master tip); the union rises from 402 to 404 tuples (the Application router's guard-constant count rises by the two new rails), so the full-file md5 and the serverSha currency pins in `routeGate.api.test.ts` and the size/provenance pins in `routeGate.harness.test.ts` all advance in the same commit, and the contracts-tip cites move. `dist/` churns by design — the shipped `client.ts` changed. Suite unchanged at 294 files (186 targeted music/gate tests re-green; full run passes). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **CI: the `test` job is now a 2-shard vitest matrix; typecheck+lint split to `static`; coverage merged from blobs (S459).** The single ~5.3-min `test` leg was the workflow's critical path, and 48-core box profiling showed the suite parallelises cleanly (19.2 s wall, `--no-isolate` FAILS 287/293 files — isolation stays ON). The split honours the S176/S231 anti-neutering gates literally: `dist-build-gate.test.ts` and `theater-geometry-gate.test.ts` parse `ui-ci.yml` as TEXT and pin the chromium install, `npm run build` → `npm run dist:check` and `npm run test:run` INSIDE the `test:` block, so every pinned step stays there on every shard leg (chromium installs on both legs — `--shard`'s file distribution is a Vitest implementation detail, and the gate's no-`if:` rule is what makes re-shuffling safe). The unpinned `typecheck`/`lint` moved to a parallel `static` job, and the Codacy lcov upload moved out of `test:` into the `coverage` job (each shard leg keeps `--coverage` on purpose — the guards require the pinned step verbatim, and the blob embeds the v8 payload, so the per-leg text report is harmless overwritten I/O). Each leg runs `npm run test:run -- --coverage --shard=N/2` with `--reporter=blob`; a new `coverage` job (`needs: test`) downloads both blobs, regenerates one report via `npx vitest --merge-reports`, and uploads the SAME lcov to Codacy — box-verified: merged `All files` percentages are byte-equivalent to a single unsharded run, shards measured 38–42 s at 4-worker emulation vs 65–77 s unsharded. Two pins added against a hazard the prototype review surfaced: the vitest step MUST stay an unnamed single-line `- run:` step (a named/`>-`-folded form makes the S231 gate's `indexOf('- run: npm run test:run')` anchor return −1 and its continue-on-error check silently vacuous), pinned by new `src/test/ci-shard-gate.test.ts` (7 tests, single code home for the lane token — including a pin that the shard blob output dir `blob/` stays gitignored, a review-surfaced local-`dist:check` footgun). Suite 293 → 294 files, 5 872 → 5 879 passed / 10 skipped; `dist/` byte-identical (workflow + test-only change, drift 0). **Ops note:** branch-protection required-check `test` no longer exists as a name — re-point to `test (shard 1/2)`, `test (shard 2/2)` and `static` after merge.

- **Route-manifest provenance re-pin — cs#42 currency cascade (2026-09-12), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #29 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is four commits of SyncPlay bridge/worker/room and WebSocket plumbing work, a catalog-pin bump and an AGENTS.md theming paragraph, with an empty diff on the two route-bearing guard-constant files, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#41 currency cascade (2026-09-12), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #28 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is one merge confined to the parallel-suite junit-merge tooling, two new guard tests and changelog/release-journal prose, with an empty diff under the route-surface directories, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#40 currency cascade (2026-09-12), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #27 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is three merges confined to parallel-harness suite-runner plumbing, an ignore-rule tidy and changelog/release-journal prose, with an empty diff under the route-surface directories, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#39 currency cascade (2026-09-11), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #26 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is one packaging-and-installation merge confined to the install script, release-process and changelog journals, and one third-party clone pin-guard test, with an empty diff under the route-surface directories, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#38 currency cascade (2026-09-11), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #25 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is two commits confined to CI workflow definitions, docker example files, helm chart value files, three support test files and changelog prose, with an empty diff under the route-surface directories, and the route-array bytes of the vendored file measure literally unchanged old-vs-new — only the two provenance lines move); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` byte-identical after a full rebuild (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#37 currency cascade (2026-09-11), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #24 against the current phlix-server master tip — route purity re-proven before regenerating: the server span since the previous pin is two commits confined to CI workflow definitions, one CI prerequisite script and three test files, with an empty diff under the route-surface directories, and the provenance-stripped fence digest measures equal old-vs-new); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` untouched (drift 0). No survival-token home in this repo — the wave token stays in its two verified code homes.

- **Route-manifest provenance re-pin — cs#36 currency cascade (2026-09-11), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #23 against the current phlix-server master tip — route purity re-proven before regenerating: the only server-side delta since the previous pin is a single test-only change (a transcoding test race fix plus changelog prose) that touches no route-registration file, and both generator-input guard blobs hash identically at both ends); 402 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` untouched (drift 0).

- **Route-manifest regen with a route add — cs#35 currency cascade (2026-09-11), NOT pure.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #22 against the current phlix-server master tip). This span is a genuine route change, not a provenance-only re-pin: server change S73 registers a new WebPortal read for a person's photo, so the manifest total moves 401 → 402 tuples (Application 365 and shared 11 hold; only the WebPortal count and total shift) and the provenance-stripped route-content digest and sorted-tuple fence digest move as intended. Currency pins advance in the same commit — the full-file md5 and full serverSha in `routeGate.api.test.ts`, the tuple-count and provenance-total pins plus the union source split (WebPortal 47 → 48) in `routeGate.harness.test.ts`, and the surface-size cites in `routeGateServer.ts` (whose shipped declaration re-emits the doc comment, so `dist/api/test/routeGateServer.d.ts` moves with it) — together with the contracts-tip cites in both gate files. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged.

- **Route-manifest provenance re-pin — cs#34 currency cascade (2026-09-10), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #21 against the current phlix-server master tip — route purity re-proven before regenerating: the routing surface from the previous pin to the new target is byte-for-byte unchanged, the only server-side deltas since the previous pin sitting in the S256 static-analysis work under scripts/config/tests and the S72 person-keyed shared artwork cache under Media/Metadata and Media/Library plus CI workflow definitions, all touching no route-registration file, and both generator-input guard blobs hash identically at both ends); 401 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` untouched.

- **Route-manifest provenance re-pin — cs#33 currency cascade (2026-09-10), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #20 against the current phlix-server master tip — route purity re-proven before regenerating: the routing surface from the previous pin to the new target is byte-for-byte unchanged, the only server-side deltas since the previous pin sitting in CI workflow definitions, changelog prose and test-support scripts with no route-registration files, and both generator-input guard blobs hash identically at both ends); 401 tuples unchanged, the provenance-stripped route-content md5 and the sorted-tuple fence digest measure equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` untouched.

- **Route-manifest provenance re-pin — cs#32 currency cascade (2026-09-10), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #19 against the current phlix-server master tip — route purity re-proven before regenerating: the routing surface from the previous pin to the new target is byte-for-byte unchanged, the only server-side deltas since the previous pin sitting in media storage + the rebuilt asset bundle with no route-registration files, and both generator-input guard blobs hash identically at both ends); 401 tuples unchanged, the provenance-stripped route-content md5 measures equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` untouched.
- **Route-manifest provenance re-pin — cs#31 currency cascade (2026-09-10), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master (untagged regen #18 against current phlix-server master tip — span from the previous pin re-proven route-zero before regenerating: the server `include` + `src` + `public` diff is empty and both generator-input guard blobs hash identically at both ends); 401 tuples unchanged, the provenance-stripped route-content md5 measures equal old-vs-new, only provenance bytes move. Currency pins in `routeGate.api.test.ts` (full-file md5 + full serverSha) and the contracts-tip cites in both gate files advance in the same commit; counts unchanged. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags and this wave is untagged. Test-only + vendored-JSON change; `dist/` untouched.
...
- **Route-manifest provenance re-pin — cs#28 currency cascade (2026-09-09), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-identical from `@phlix/contracts` master `a1ca39d8` (untagged regen against server master `a5cde27e`); 401 tuples unchanged, route bytes unmoved — only provenance moves. Currency pins in `routeGate.api.test.ts`: full-file md5 `5c06306c` → `0331a2d8`, serverSha `afe54c7c` → `a5cde27e`; docblock contracts cites in `routeGate.api.test.ts`/`routeGate.harness.test.ts` move to `a1ca39d8`. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` pin tracks tags, this wave is untagged. `dist/` byte-identical (build + `dist:check`, 0 drift entries).

- **Route-manifest provenance re-pin — cs#27 currency cascade (2026-09-09), PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored verbatim from `@phlix/contracts` master `28000fa4` (regen against server master `afe54c7c` — seven route-zero server merges since `1e14b539`); all 401 tuples byte-identical (stripped route-set md5 `508a6415` old = new) — only provenance moves. Currency pins in `src/api/test/routeGate.api.test.ts`: full-file md5 `e3647899` → `5c06306c`, serverSha `1e14b539` → `afe54c7c`; docblock contracts cites in `routeGate.api.test.ts`/`routeGate.harness.test.ts` move to `28000fa4`. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` install pin tracks tags, this wave is untagged. `dist/` byte-identical (build + `dist:check` on the clean tree, 0 drift entries).

- **Route-manifest re-vendor — cs#26 currency cascade (2026-09-08), NON-PURE.** Vendored `src/api/test/server-route-manifest.json` re-vendored byte-exact from `@phlix/contracts` master `97c87f27` (regen against server master `1e14b539` — the S273 route add): 400 → **401** tuples, `POST /api/v1/admin/updates/check` joins the set. Currency pins in `src/api/test/routeGate.api.test.ts`: md5 `4f4dc687` → `e3647899`, serverSha `2746677e` → `1e14b539`, length 401; harness count/total pins and the `routeGateServer.ts` surface prose move 400 → 401; docblock contracts cites move to `97c87f27`. `contractsPin.test.ts` UNTOUCHED — the `#v0.4.6` install pin tracks tags, this wave is untagged. The `dist/` churn this commit carries is the S273 card copy, recorded in the Added entry above.

- **Route-manifest provenance re-pin — cs#25 currency cascade (2026-09-08).** Vendored `src/api/test/server-route-manifest.json` re-vendored verbatim from `@phlix/contracts` master `e837e31c` (regen against server master `2746677e`); all 400 tuples byte-identical — only provenance moves. Currency pins in `src/api/test/routeGate.api.test.ts`: md5 `b6acafdf` → `4f4dc687`, serverSha `df6aa8e5` → `2746677e`; docblock contracts cites in `routeGate.api.test.ts`/`routeGate.harness.test.ts` move to `e837e31c`. `dist/` untouched. Untagged wave: no version or install-pin moves.

- **`@phlix/contracts` install-pin re-pin — `v0.4.5` → `v0.4.6` (S447).** `package.json` moved its `github:detain/phlix-contracts` dependency from the stale `#v0.4.5` tag to `#v0.4.6`, and `package-lock.json` re-resolved its single `@phlix/contracts` entry to the commit that tag peels to — the retirement of S442's accepted ui debt, re-numbered correctly. The premise of a stale *nested* copy was refuted on disk: the lock held exactly one resolution and that direct pin *was* the stale copy. A new pin guard test (`src/api/test/contractsPin.test.ts`) now asserts the tree resolves `@phlix/contracts` exactly once and to the `v0.4.6` tag peel, failing any re-introduction of the stale pin or a sha-pinned dependency string. The vendored server route-manifest and its currency md5 are deliberately untouched — those bytes track the cs-cascade, not this npm pin. `dist/` byte-identical (no runtime surface change; the `v0.4.5`→`v0.4.6` delta is declaration/provenance).
...
- **Route-manifest provenance re-pin — cs#22 currency cascade (2026-09-06).** The vendored `src/api/test/server-route-manifest.json` is re-vendored verbatim from `@phlix/contracts` master `876d0ea` (regen against server master `e4853f0f`; previous provenance `341fc6e2`/`e729d48a`). All 400 tuples byte-identical — only provenance moves — currency pins in `src/api/test/routeGate.api.test.ts` follow: md5 `7accd31d` → `791235d4`, serverSha `e729d48a` → `e4853f0f`; docblock contracts cites move to `876d0ea`. `dist/` untouched. Untagged wave: no version or install-pin moves.

- **Relay inbound ms→s conversion at the protocol boundary (finish-S293, S441).** Outbound positions have carried MILLISECONDS since S293 (SPEC.md:91) but inbound frames were consumed raw, so a host's 300 000 ms re-anchor landed as 300 000 seconds and dragged `video.currentTime` by 1000×. One decode — `wireMsToSeconds()` in `src/api/syncplay.ts` — now normalizes every inbound position to the UI-internal seconds at the three boundary legs: `onPlaybackCommand`, `onPlaybackSync`, and the REST join's `groupToSession()`. Store and Player stay seconds-consumers (zero math below the adapter); the wire fixtures in `syncplay.ws/.mappers/.routes/.anchor/.actions/.join` now feed ms frames and assert seconds, 1000×-sensitive both directions (mutation-proved). Send boundaries untouched.
- **Route-manifest provenance re-pin — cs#21 currency cascade (2026-09-06).** The vendored `src/api/test/server-route-manifest.json` is re-vendored verbatim from `@phlix/contracts` master `341fc6e2` (regen against server master `e729d48a`; previous provenance `f2e284b3`/`f35a5742` — the cs#20 web-ui re-tag leg: zero route hunks). All 400 tuples are byte-identical — only provenance moves — and both currency pins in `src/api/test/routeGate.api.test.ts` follow: vendored md5 `05db9e75` → `7accd31d` and serverSha `f35a5742` → `e729d48a`; the docblock contracts cites in `routeGate.api.test.ts` and `routeGate.harness.test.ts` move to contracts `341fc6e2`. No route change, no source change: the gate's covered-URL pins are untouched and the built `dist/` stays byte-identical. Untagged wave: no version or install-pin moves.
...
- **Four follow-ups to the S82 profile surface (S462–S465).** S462: the UserMenu "Switch Profile"/"Manage Profiles" arms now render only when the profiles feature is enabled — the flag (`features.profiles ?? app !== 'hub'`) moved from an inline `PhlixApp` expression into one shared helper (`src/app/profilesFeature.ts`) consumed by both; enabled behavior is pinned unchanged (default config keeps the arms) and new tests hide both arms under `features.profiles:false` and `app:'hub'` with the rest of the menu intact. S463: `useProfileStore.removeProfile` deleting the ACTIVE row previously nulled the scope (the epoch watcher bumps id→null) then adopted the survivor via null→id — a transition the watcher deliberately skips — so epoch-scoped listeners' last re-read happened on an anonymous scope, never for the healed one; adoption now bumps the epoch explicitly (mutation: removing the bump reddens the named test with "expected 1 to be 2"). S464: the active-profile localStorage key was ONE global `phlix.active_profile` — shared browsers leaked one account's first-paint hint to every other — and is now namespaced `phlix.active_profile.<account-id>` via exported `activeProfileStorageKey()`, writes skipped while the account is unknown, an account-arrival watch seeding a null mirror only (never clobbering a live scope, never bumping epoch). COMPAT = documented RESET: the legacy global key is removed on store boot and never fallback-read — the value is a non-authoritative paint hint (`load()` adopts server `is_active` truth within a request), so a fallback-read would re-import the exact cross-account leak; estate census: only `useProfileStore.ts`, its test and the `src/index.ts` re-export ever touched the key, zero hits in phlix-server/phlix-hub — no cross-repo follow-up. S465: the S82 test stubs emitted the 409 last-profile refusal with the machine code in `message` only, while the REAL server (`ProfilesController::delete`) answers 409 `{error:'profile.last_profile', message:'Cannot delete the last profile'}` and the client's `extractError` PREFERS `error`; stubs in `useProfileStore.test.ts` and `ProfilesPage.test.ts` now build both keys from new exported contract constants in `src/api/admin/users.ts` (`PROFILE_LAST_ERROR_CODE` / `PROFILE_LAST_ERROR_TEXT` / `LastProfileConflictBody`), and the ProfilesPage test additionally asserts the human text is NOT displayed (proves key precedence). The plan called this a "conflicting profile-switch/409" — the switch endpoint has NO 409 path server-side (400/401/404 only): code won, the estate's one profile-409 is the delete refusal. BrowsePage's favorites-rail assertion tightened from ≥2 to exactly 2 `favCalls` (mount read + epoch-swap re-read). Suite 5 879 → 5 889 passed / 10 skipped (+10 tests); `dist/` rebuilt and committed, `dist:check` green.
### Fixed

- **A released S76 changelog bullet lost under W110 editing is restored, and the chrome-focus watcher now names its parameter truthfully (W110 review repair).** A W110 step first inserted its entry over the S76 lead bullet under the released 0.98.37 section, and the follow-up that relocated that entry removed its own line but never gave the bullet back — so S76's ⚠ caveat, its “an error is not no-update” and its parser-defaults paragraphs sat orphaned under a bare `### Added`. The bullet is restored byte-exact from the pre-W110 file and the three paragraphs read as its continuations again. Same sweep: the `showChrome` watcher in `Player.vue` received the *showing* value in a parameter named `hidden` — the name contradicted the truth table its own branches honour, so it is renamed `visible`, behaviour untouched. Two double-blank seams left by the W110 insertions (`Player.vue` after the stall-watchdog canceller, `transcode.ts` after the profile-cap mapper) are collapsed. `dist/` changes by design (the shipped component changed — its scoped-style id moves with it).
...
- **Route-manifest provenance re-pin — cs#18 currency cascade (2026-09-05).** The vendored `src/api/test/server-route-manifest.json` is re-vendored verbatim from `@phlix/contracts` master `51ed6cd3` (regen against server master `e74cdc88`; previous provenance `4b620f59`). All 400 tuples are byte-identical — only provenance moves — and both currency pins in `src/api/test/routeGate.api.test.ts` follow: vendored md5 `81eeef82` → `9f69628d` and serverSha `4b620f59` → `e74cdc88`. No route change, no source change: the gate's covered-URL pins are untouched and the built `dist/` stays byte-identical. Un-tagged wave: the `@phlix/contracts#v0.4.5` install pin stays.

- **Route-manifest provenance re-pin — cs#17 currency cascade (2026-09-04).** The vendored `src/api/test/server-route-manifest.json` is re-vendored verbatim from `@phlix/contracts` master `55311c68` (regen against server master `4b620f59`; previous provenance `888a42b2`). All 400 tuples are byte-identical — only provenance moves — and both currency pins in `src/api/test/routeGate.api.test.ts` follow: vendored md5 `9727f2d3` → `81eeef82` and serverSha `888a42b2` → `4b620f59`. No route change, no source change: the gate's covered-URL pins are untouched.

- **The route gate re-adopted the canonical `phlix-contracts` route-manifest export (S280, W19).** The ui deleted its own generator and its generated TS (`scripts/generate-server-route-manifest.mjs` + `src/api/test/serverRouteManifest.generated.ts`) — two derivations of one truth is exactly the drift class S280 exists to kill. The canonical export is now vendored verbatim: `phlix-contracts` `dist/server-route-manifest.json` → `src/api/test/server-route-manifest.json`, md5 byte-identity verified against the contracts artefact at `09161041` (short form `cca4660d…`). The gate re-pinned **389 → 400 tuples** @ server `8f72faec…` (delta: 0 removed, 11 added — the ten S81 profiles self-service rails from server #707 plus `POST /auth/identities/link/hub`; the old set was verified a strict subset, so every per-module pin in the Added entry stands unchanged). Two currency pins were added — an md5 assertion and a full-provenance sha assertion — both mutation-proved red→green. The Application source count moved 353 → 364 and `shared: 11` is now pinned. The vendoring is the sanctioned interim: the ui pins `@phlix/contracts#v0.4.4`, which predates the deliberately-untagged export, and the contracts `exports` map blocks JSON subpath imports — this repo switches to the tagged import when the next tag lands. The `--server-sha` / `parseTuple` generator hardening now lives canonically in phlix-contracts (the ui-local copy lacked it). The app bundle is provably untouched: a full `npm run build` + `dist:check` is green with zero JS/CSS asset drift — only the two `dist/api/test/*.d.ts` declaration entries moved — and `test:run` reports 5778 passed / 10 skipped (+1 currency test).

- **`@phlix/contracts` pinned to v0.4.4 (S325 consumer bump).** The v0.4.4 tag carries the canonical snake_case parental-controls create shape (S234) and admits `dash_url` on the transcode shapes the server emits (S325). The SPA previously pinned v0.4.3. The bump is type-only for this repo: `vue-tsc` clean, `test:run` 5777 passed / 10 skipped (the existing set), and `npm run build` re-emitted `dist/` **byte-identical** (0 drift) — no source change was required for the new types.

### Added

- **Every URL the client issues is now pinned tuple-exact against the server's ROUTE_MANIFEST — the S280 route gate.** Until now nothing in any repo pinned what a client *requests*: `ROUTE_MANIFEST` pins only what the server *registers*, so a client could call a URL that never existed with every test in both repos green (the S264 `/members`, S279 tizen, and `/rooms` drift classes). The gate derives its expected set from phlix-server ONLY — since the W19 re-adoption (see the Changed entry above) via the canonical phlix-contracts route-manifest export vendored verbatim; W17 originally derived it repo-locally (ApplicationRouter 353 + WebPortalRouter 47, 11 shared → 389 tuples) from a committed ui-local generator that W19 deleted. `makeRouteGateServer` answers a real **404** to any url outside that set (the S276 prototype generalised; `{param}` matches exactly one segment — sibling-wildcard absorption is impossible) and the gate drives every request-issuing `src/api` module plus the stores/composables that mint URLs: no-404 + exact per-module covered-URL sets (client.ts 35, admin users 26, liveTv 21, plugins 18, remoteAccess 17, libraries 16, integrations 13, cast 11, collections 9, backup/maintenance 8/8, webhooks 5, syncplay 5×2, dashboard 5, photos 5, metrics/transcoding/history 4, dlnaServer/logs/settings/services 3, duplicates/networkHealth/servers 2, themes/recommendations/nextUp/mostWatched/libraries/letter-index/index-buckets/metadata-sources/updates 1, auth store 3, media store 2, resume reporter 3, user-item-data 5, hls transcode 2, music-player 1, series-seasons 1, trickplay 1, resume-sync 1). A mechanical enumeration gate scans `src/api` + stores + composables and asserts every request-issuing module is driven or explicitly partitioned — hub-addressed surfaces (claimServer, invite-links, mcp-tokens, users relay bandwidth, `servers/{id}`, hub dashboard summary) and known-unserved endpoints (transcoding accelerator setter, webhook delivery logs) are enumerated with a named **W18 follow-up** (contracts export + the other clients' gates + server-side fixes for the unserved pair).

### Fixed

- **`ApiClient.addToPlaylist()` called `POST /api/v1/playlists/{playlistId}/items` — a URL the server never registered** (the S280 gate caught it on the current tree). The server's playlist alias creates a **collection** (`POST /api/v1/playlists` → `CollectionController::create`), so item-add is the collection route: the method now calls the registered `POST /api/v1/collections/{id}/items/{mediaItemId}`.
- **`AdminLiveTvApi.scanTuners()` POSTed `GET /api/v1/admin/livetv/tuners/scan`** — the server registers the rail as a GET (S280 gate). The client now GETs it, and the page/API tests assert the corrected verb.
- **Card hover-lift on home rails is no longer clipped at the rail's top edge (S419).** `.media-row__rail`'s `overflow-x: auto` makes it a scroll container on both axes (CSS Overflow 3), and with zero top padding the 11.00px overhang of `MediaCard`'s hover lift — measured by S222's headless-Chromium probes, the defect S222 filed — was invisible on every rail. The rail now carries a 16px top clearance INSIDE its scroll box and gives the identical room back OUTSIDE via a negative-margin compensation in the same rule, so the cards' on-page position is pixel-unchanged and the inter-rail rhythm is untouched. Two-sided built-stylesheet guards pin the mechanism, the bottom gutter, the head rhythm partner, and the lift transform itself (reverting any single lever turns its named test red).

- **The hub relay's `pending_command` now has a real consumer: "Alexa, play X" can reach an open app (S298).** New `src/api/hubRelay.ts` opens a socket to the hub's SyncPlay relay (`ws(s)://<hub>:8804/syncplay/{server_id}`) with the token on the `Sec-WebSocket-Protocol: bearer, <token>` subprotocol — the only carrier a browser WebSocket can present (S237 removed the query-string carrier the old sockets relied on). The socket opens at app boot (via `PhlixAppConfig.hubRelay`), NOT inside a SyncPlay room join — the hub delivers to an authenticated (user, server) socket regardless of room membership, which is the primary "no room open" case. It survives with a capped reconnect ladder that re-reads the token each attempt (relay tokens expire hourly) and rebinds cleanly when the app switches servers. Only `pending_command` / `play_media` frames are consumed (`parsePendingCommandFrame` at the boundary); every other relay frame is ignored.
- **`SyncPlaySession.currentMediaId` is now carried through `groupToSession()` and has a caller (S298).** The mapper previously discarded the wire's `current_media_id`; it now maps to `currentMediaId` (`null` when absent). The store's new `applyPendingPlayMedia()` — the hub-consumer path — writes the delivered media id into a live session, and Player.vue's load path consumes it, so the field is produced and read by real code. (The server-side `GroupState::setCurrentMedia()` zero-production-caller gap is a phlix-server item; the wire value maps faithfully until a server lane wires it.)
- **Player.vue gained a load-a-new-title path (S298).** The SyncPlay watcher was transport-control-only (play/pause/seek on an already-loaded element). A delivered `pending_command` now resolves its media id through the new `resolvePendingMedia` prop, `setCurrent()` loads the title, `play()` starts it, and the store slot is consumed; without a resolver the command is surfaced once via the new `pending-media` event for the host to route. A stale resolution (a newer command landing while the first is still resolving) is discarded, and a rejecting resolver falls back to the event.
...
- **S19's no-backdrop ambient scrim was theme-INVARIANT while the hero text is theme-FOLLOWING, so it made the light (Daylight) theme worse (S326).** The scrim's 0%/35% gradient stops were fixed `rgba(0,0,0,0.55)` / `rgba(0,0,0,0.35)` — only the 100% stop followed `var(--bg)` — so on Daylight a fixed dark wash sat behind DARK `--text`/`--text-muted` hero text. Settled by rendering the real `MediaDetail` in headless chromium and measuring the COMPOSITED pixels behind the title/meta/overview (9 samples per element per theme): Daylight meta measured **2.40:1** against the required **4.5:1** — the exact opposite of S19's "keep the hero legible" goal. Every stop is now theme-derived — `color-mix(in srgb, var(--bg, #000) 55%/35%, transparent)` for the 0%/35% stops on BOTH the ambient scrim and the backdrop scrim it mirrors — so the scrim darkens on Nocturne/Midnight (near-identical to before) and LIGHTENS toward the page on Daylight. Post-fix rendered minima: Daylight title **11.77:1** (≥3 large text), meta **4.84:1**, overview **5.26:1**; Nocturne 16.53/8.62/8.74; Midnight 16.50/7.70/7.83 — all ≥ their WCAG thresholds. The meta row's `.media-detail__type` chip was also measured at 4.18:1 on Daylight (it used `--text-subtle`, which passes its token lock on plain surfaces but not over the hero scrim) and now shares the row's `--text-muted`. The two comments that claimed the opposite (the S19 title-shadow comment and the test's "theme-following" comment — true of the 100% stop only) are corrected. Pinned by a new in-suite chromium render test (`src/test/ambient-scrim-contrast.browser.test.ts`, theater-geometry pattern — real pixels, real `@phlix/tokens` stylesheet, worst text color per box, every sample printed) plus a source guard that fails on the pre-fix fixed stops, both wired into the anti-neutering gate.

- **The music count lines were hand-rolled plurals that the repo's own lint rule could not see (plan_updates.md — S346).** Eleven ternaries of the shape `count === 1 ? t('music.XTotalOne') : t('music.XTotal', { count: n.toLocaleString() })` — across `MusicAlbumCard.vue`, `MusicArtistCard.vue`, `MusicAlbumPage.vue`, `MusicArtistPage.vue` (×2), `MusicArtistsPage.vue` (×2), `MusicLibraryPage.vue` (×2) and `MusicTracksPage.vue` (×2) — are now single pipe-form calls `t('music.XTotal', { count: n.toLocaleString() })`. The four `*One`/`*` catalogue pairs in `src/i18n/messages.ts` were collapsed into their pipe-form templates (`artistsTotal`, `albumsTotal`, `tracksTotal`, `tracksOnPage`) and the `*One` keys deleted. `createTranslator` → `selectPlural` (driven by `Intl.PluralRules` since S134) picks the right form; a pre-grouped count string like `'2,197'` degrades to the last (plural) part, so the rendered output is byte-identical to before ('1 album', '2,197 artists', '1 track', '0 tracks').

  **The rule's blind spot was structural.** `isStringish()` stops at `CallExpression`, so a ternary whose branches are `t()` calls was invisible to `plural/no-hand-rolled-plural`. A targeted check now fires when a conditional compares something to 1 AND both branches are bare `t()` calls — the keys ARE the strings being selected, routed through the translator. It is deliberately narrow: accepting any `CallExpression` branch was measured to produce six false positives (`MediaGrid.vue:288`, `player/ambient.ts:88+101`, `DashboardPage.test.ts:329`, `MetadataMatchModal.test.ts:180`, `MusicLibraryPage.test.ts:675`), so the check requires the exact `t` identifier and both branches. `src/utils/plural.ts` is unchanged — the pipe-form machinery already existed; only the call sites, the catalogue and the rule moved. Rule tests grew from 54 to 67.

- **The Libraries admin page knows the 9th `library_scan_jobs.type` member — `media_assets` (S341).** Migration 101 (S284) appended `media_assets` to the server ENUM, but `SCAN_JOB_TYPES` in `src/api/admin/libraries.ts` still carried 8 members, so `ScanJobType` was a wrong type and the Type cell in the scan history fell back to the raw snake_case value. `media_assets` is now APPENDED last (append-only — MySQL stores ENUMs by index, inserting in the middle re-numbers existing rows), `SCAN_TYPE_LABELS` in `LibrariesPage.vue` renders it as "Media assets" (`Record<ScanJobType, string>` stays compile-complete: a missing label is a compile error), and the literal pin in `LibrariesPage.test.ts` grew its 9th member. The pin stays deliberately literal (a check derived from its own subject self-adjusts), so the decay is closed on the SERVER side: phlix-server's `ScanJobsEnumMigrationTest` parses the real migration 101 through `MigrationRunner::splitStatements` and asserts `ScanJobRepository::ALLOWED_TYPES` equals the parsed ENUM, naming this repo's `SCAN_JOB_TYPES` as the mirror in its failure message — the next member addition goes red instead of rendering a bare label.

- **The Libraries admin page can now trigger S284's media-asset regeneration (S348).** `AdminLibrariesApi` shipped 8 maintenance methods (`scan` … `delete-all`) and no `regenerateAssets`, so an install scanned before the trickplay-producer fix could never re-prime chapter thumbnails / trickplay sprite / Roku BIF short of a full rescan — the admin UI simply had no way to call the server route `POST /api/v1/libraries/{id}/regenerate-assets`. The 9th method is added consuming the same `ScanQueuedResult` 202 shape (its `status` union is now `'queued' | 'already_queued'` — a second request while the `media_assets` job is active is an idempotent SUCCESS, not an error), the Libraries row's overflow menu gains "Regenerate media assets" (non-destructive, so no confirm dialog) with a matching help-disclosure entry, and the API + page tests cover both arms. The hand-written method list is no longer the only guard: the decay is closed on the SERVER side — phlix-server's `LibraryMaintenanceRoutesUiMirrorGuardTest` parses the production route table, extracts the library-maintenance POST-route set, and asserts it equals this class's method list in both directions, naming `AdminLibrariesApi` as the mirror to update when a maintenance route is added without a UI method.

- **The Search results grid finally wires the admin ⋯-menu metadata actions (S324).** `SearchPage` rendered `<MediaGrid>` with no `@edit-metadata` / `@explore-data` listeners and mounted neither `MetadataMatchModal` nor `ItemDataInspector` — the one `MediaCard` host S15's In-scope list omitted, so an admin's "Edit metadata" and "Explore item data" ⋯-menu items were dead on `/app/search` (the menu is built by `MediaCard` itself, with no host gate). The grid now binds all three ⋯-menu-emitted events (`@edit-metadata` / `@refresh` → the shared metadata-match modal, `@explore-data` → the read-only inspector) plus the admin-gated "Match" quick action, mirroring the five wired hosts. Because `SearchPage` is the one long-tail page that statically imported a deferred surface, its `src/index.ts` export became a `defineAsyncComponent` factory — the `MetadataMatchModal` chunk stays out of the eager boot graph (UI-3.1 GAP 1) and the page matches the R6.1a lazy-route precedent. A new runtime host enumeration (`hosts-admin-actions.test.ts`) scans `src/pages/**/*.vue` for every emit-capable media component and fails on any host missing the listeners or either modal, printing the 6-page host set it examined; wiring `@refresh` on `ExplorePage` / `RecommendationsPage` (the same dead item there) closed the last two menu items. Covered by RED-on-unwired tests in `SearchPage.test.ts` plus sibling `@refresh` tests on the other two hosts.

### Added

- **Admin → Tasks: the four absent sections from S78's eight-section brief are now each decided by name, and the three owned by no step carry honest no-endpoint notes (S330).** The AC audit found S78's In-scope list named eight sections while only four shipped, and that an AC narrower than its own In-scope list is a mis-specified step. Three of the four missing sections were owned by nothing (scan-all, recompute-similarity, newsletter send-now — "check for updates NOW" is S273-owned). S330 closed each by name with a written reason rather than leaving it silently missing: **scan-all** (no server-wide scan service; per-library scans on the Libraries page are the bounded path — the existing honest note stays), **recompute-similarity** (similarity is computed per item in the background as media is scanned by the live `SimilarityWorker`; a server-wide recompute is O(N²) per library and unwanted — the page now says so), **newsletter send-now** (the newsletter is sent on the server's weekly schedule; a send-now is unwanted — the page now says so). No new buttons, no new API methods and no `MAINTENANCE_TASK_NAMES`/`MAINTENANCE_ENDPOINTS` changes ship — a button that 404s is worse than an absent one, so the honest copy is the deliverable. Cross-repo: phlix-server's new `AdminMaintenanceRoutesUiMirrorGuardTest` pins the route surface against this repo's `AdminMaintenanceApi` lists in both directions, so a future server route added without a UI mirror (or vice versa) goes red.

- **`RatingBadge`, `UserRatingPicker` and `ProfileImageSettings` are exported — they were unshipped work, not dead code (S263).** All three lived in `src/components/` with no `src/index.ts` export and no importer, and were therefore **tree-shaken out of every published artefact**: `rating-badge`, `user-rating-picker` and `profile-image-settings` each occurred **zero** times in `dist/phlix-ui.js`, `dist/ui.css` and `dist/style.css`, while exported siblings `phlix-icon` and `thumb-rating` occurred once each — the control that makes that zero a result rather than a search that never ran. No consumer could reach them even by deep path: `files: ["dist"]` ships no SFC source, so the only artefacts that shipped were three orphan `dist/components/*.vue.d.ts` type declarations describing runtime modules that did not exist. Exporting is therefore **purely additive** — nothing could have been importing them, so nothing can break.

  **Deletion was the other option and was rejected on evidence, not taste.** `ProfileImageSettings.vue` is the only caller in `src/` of `useAuthStore().uploadAvatar()` / `.deleteAvatar()`, which are in turn the only callers of `ApiClient.uploadAvatar()` / `.deleteAvatar()` — one complete, 100%-covered vertical slice with no UI entry point, whose deletion would have orphaned the two layers beneath it. `phlix-docs/docs/libraries.md` documents `<RatingBadge>` as a public `@phlix/ui` component. And four native clients have each hand-rolled a clone of the star pair (`phlix-tizen-client/src/components/{RatingBadge,UserRatingPicker}.vue`, `phlix-mobile-client/src/components/{RatingBadge,UserRatingPicker}.tsx`, `phlix-roku-client/components/RatingBadge.brs`, `phlix-console-client/src/Ui/{RatingBadge,UserRatingPicker}.php`) — the estate wants this surface; the shared library is where it belongs.

  **Their 100% coverage was NOT the evidence.** S182 took all three from `LH:0` to full line and function cover without wiring any of them up, so coverage here measures testedness and says nothing about reachability. The reachability sweep is what decided it, and it ran over the consumer repos, not just this one.

  Pinned by `src/index.test.ts` → *"S263 — the newly exported components"*, which **mounts** each binding taken from the barrel. A `toBeDefined()` would have caught the export being deleted but not the export being repointed; a `readFileSync`-and-match test would have caught neither, which is exactly how `ProfileImageSettings.vue` read as covered at `LF:73 LH:0` — `src/__tests__/s241-image-src-coverage.test.ts` reads it as **text**, putting it in the coverage tool's field of view without executing a line.
