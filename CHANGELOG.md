...
### Added
- **The media-detail button hierarchy, the no-backdrop ambient scrim and the poster-action tooltips, audited by execution instead of by eye (plan_updates.md — S18 + S19 + S20, AC audit 2026-08-03).** Test-only — **not one byte of bundled source changes**, so `dist/` is untouched and `npm run dist:check` reports **0 drift entries**. All three steps' production changes are already on master (S18 `6109f896` PR #255, S19 `7a7e1865` PR #256, S20 `7a7d7bdf`+`807fd55a`), every in-scope bullet re-verified against the current files, and each one **holds under mutation**. All three acceptance criteria were nevertheless written as a *manual per-theme visual check across Nocturne/Daylight/Midnight*, which cannot be discharged here: `test:visual` is not part of the deterministic gate, and vitest runs with `css` disabled (`vite.config.ts`), so **a scoped `<style>` block is never injected into jsdom** and `getComputedStyle` reads `''` for every rule these steps added. What each criterion actually needed was a test that would catch the change being reverted — and for two of the three there was none.

  **S19's CSS half was pinned by nothing.** The three shipped S19 tests assert only that the scrim ELEMENT renders, so the entire `.media-detail__ambient-scrim` rule could be deleted with the whole suite green — a scrim div painting nothing, which is exactly the pre-S19 state the step exists to fix. The same was true of every hero `text-shadow`. Now asserted from the SFC source: the ambient scrim mirrors `.media-detail__backdrop-scrim`'s `background` and both spellings of `backdrop-filter` (the step's own wording is "the same scrim treatment the backdrop already has", so the assertion is *equality with the backdrop scrim*, not a hard-coded gradient literal), it matches the wash's `inset`/`height`/`z-index`/`mask-image`, it is `pointer-events: none`, and — since both layers sit at `z-index: 0` — the DOM order that makes it paint *above* the wash. Title, meta and overview must each declare a real black-halo `text-shadow` with a non-zero blur and at most a 1px offset (soft, not embossed). Seven mutations, one named failure each: deleting the scrim's `background`, deleting the whole rule, `height: 60vh → 40vh`, swapping the two divs, deleting the title shadow, `meta → none`, and overview `blur: 2px → 0px`. The parser strips comments and every at-rule block first, so a declaration that exists only inside `@media (…)` can never satisfy an assertion about the unconditional rule.

  **S20's hover reveal was pinned by nothing either, and the reason is instructive: the primitive *was* tested, in a file named after a different component.** `Tooltip.vue`'s only tests are a `describe('Tooltip')` block inside `src/components/ui/Modal.test.ts`, and both are blind to this criterion — one shows on FOCUS with an explicit `delay: 200` on a synthetic `<button class="trg">`, the other asserts a DISABLED tip never opens. So the **300ms default was asserted nowhere**, no test connected a poster action button to a tip that actually opens, and the hover path had no coverage at all: **deleting `@mouseenter="show"` from `Tooltip.vue` left all 239 files / 4,464 tests green**, and now fails exactly the two new hover tests. The reveal is now asserted at the 299ms/300ms boundary on both a `MediaCard` action and a `ThumbRating` thumb, on hover *and* on focus (hover is dispatched on the wrap because `mouseenter` does not bubble; focus is dispatched on the BUTTON, which is what a Tab press hits, and reaches the wrap because `focusin` does), together with the `aria-describedby` wiring and its release on leave/blur, and `delay === 300 && disabled === false` on all 8 tips.

  **The second half of S20's criterion — "the card action-row width math is unaffected" — is not measurable in jsdom and is not claimed to be.** `getBoundingClientRect`, `offsetWidth` and `offsetHeight` all report **0**, and no CSS is applied, so no assertion here can show four 32px buttons fitting a 140px row. What is pinned instead is everything the math depends on: the row has exactly **7 flex items** and every one is a `.phlix-tooltip-wrap` / `.thumb-rating` / `.phlix-menu` (an extra `<span>` around one button is red), plus the `max-width: calc(4 * 32px + 3 * var(--space-1))` and `:deep(.phlix-tooltip-wrap) { flex: 0 0 auto }` declarations themselves. The step's *conditional* `placement="bottom"` suggestion resolves to keeping the default `top`, and that decision is now pinned together with the two CSS facts behind it (`.media-card__poster { overflow: hidden }` + `.media-card__overlay { justify-content: flex-end }`), so flipping one without the other cannot pass silently. **Residual, unverifiable here:** a long tip ("Remove from favorites") is `white-space: nowrap` and centred on a 32px button inside that clipping poster, so on the narrow rail cards it can still be clipped *horizontally* — `placement` cannot fix that, and jsdom's zero-width measurements cannot demonstrate it either way.

  **S18 was the healthiest of the three** — the variant hierarchy (`solid` Play only, `outline` Resume/Trailer/Match, `ghost` Watchlist/Watched), both replaced raw `<button>`s and ThumbRating's `var(--text)` were all already asserted, and all still bite (reverting the thumb colour to the hardcoded `rgba(255, 255, 255, 0.92)` is red). Three gaps closed: `Back` must stay `ghost` and Play must be the only solid button in the *whole component* (not just the action row); **no raw `<button>` may survive in the hero action row** at all — every one must carry `phlix-btn` / `phlix-iconbtn` / `thumb-rating__btn`, with the fully-populated row's 6/3/2 composition pinned, so the next control added by hand fails here rather than shipping a fourth visual weight; and every icon-only hero control must stay on the `ghost` tier with no `.phlix-iconbtn--solid` anywhere in the row (`.phlix-btn--solid` was scanned, its icon-button twin was not). **Still not verifiable by execution:** whether the three themes *look* consistent, and one property inherited from the accepted design — on the light (Daylight) theme both scrims paint a dark gradient under dark text, exactly as the pre-existing backdrop path already does. S19 explicitly puts luminance-adaptive text colour out of scope, so that is recorded, not changed.
