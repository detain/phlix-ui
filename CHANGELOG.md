...
> **Not released yet.** `@phlix/ui` is delivered by **git tag with a prebuilt `dist/` committed inside
> the tag**, and the work below deliberately left `package.json` at `0.98.31` with `dist/` and
> `public/assets/` untouched — so consumers receive none of it until the next version bump +
> `npm run build` + `dist/` commit + tag. Rename this heading to that version when the tag is cut.
