...
import { l as t, p as n, t as r, u as i } from "./client-COHWZ2KC.js";
...
import { t as oe } from "./Tabs-lzVIjdc-.js";
import { t as s } from "./HelpText-CMd9gM8h.js";
import { n as se, t as ce } from "./useSettingsPrefs-CEFxTJFG.js";
...
var he = {
	...
	setup(e) {
		let g = e, C = ue("apiBase", ""), Ye = l(() => typeof C == "string" ? C : C?.value ?? ""), Xe = g.client ?? new r({
			baseUrl: Ye.value,
			tokenStore: new t()
		}), w = new se(Xe), T = ee(), Ze = ce(), Qe = y({}), $e = y([]), E = y({}), D = y({}), O = y({}), et = l(() => Object.keys(O.value).length === 0 && Object.keys(E.value).length > 0);
		function tt(e) {
		...
		let k = l(() => {
			if (!et.value) return O.value;
			let e = {};
			for (let t of Object.keys(E.value)) e[t] = it(t);
			return e;
		}), A = l(() => {
		...
		let j = y(""), ot = y(!0), M = y(null), N = y(!1), P = y(!1), F = y({}), I = v({}), L = v({}), R = v({}), z = v({}), B = v({}), st = l(() => Object.values(z).some(Boolean) || Object.values(L).some(Boolean)), ct = l(() => Object.entries(k.value).filter(([, e]) => e.group === j.value).map(([e]) => e)), lt = l(() => Object.entries(k.value).filter(([, e]) => e.restart === !0).map(([e]) => e));
		function ut(e) {
			return k.value[e]?.restart === !0;
		}
		let V = y([]), dt = l(() => V.value.length > 0), ft = l(() => `${Je}:${Ye.value || "default"}`), pt = l(() => V.value.map((e) => J(e)));
		function mt() {
		...
		function ht() {
			if (!(typeof localStorage > "u")) try {
				V.value.length === 0 ? localStorage.removeItem(ft.value) : localStorage.setItem(ft.value, JSON.stringify(V.value));
			} catch {}
		}
		function gt(e) {
			let t = /* @__PURE__ */ new Set([...V.value, ...e]);
			V.value = Array.from(t), ht();
		}
		function _t() {
			V.value = [], ht();
		}
		...
		function wt(e) {
			...
			for (let [t, n] of Object.entries(e)) R[t] = Ct(t, n);
		}
		function Tt() {
		...
		function Et(e) {
			Qe.value = e.settings, $e.value = e.overridden, e.secretStatus && (D.value = e.secretStatus), wt(e.settings), Tt();
		}
		async function Dt() {
			ot.value = !0, M.value = null;
			try {
				let e = await w.get();
				E.value = e.types, O.value = e.meta, Et(e), F.value = {}, (!j.value || !A.value.some((e) => e.value === j.value)) && A.value.length > 0 && (j.value = A.value[0].value);
			} catch (e) {
		...
		function jt(e) {
			let t = Object.keys(k.value).filter((t) => k.value[t]?.group === e);
			return t.length > 0 && t.every((e) => K(e));
		}
		function q(e) {
			return K(e) && !Ze.advancedMode;
		}
		function J(e) {
			return k.value[e]?.label || tt(e);
		}
		function Y(e) {
		...
		function Nt(e, t) {
			...
			try {
				let n = JSON.parse(t);
				delete B[e], z[e] = JSON.stringify(n) !== Mt(e);
			} catch (t) {
				B[e] = `Invalid JSON: ${t instanceof Error ? t.message : "could not be parsed"}`, z[e] = !0;
			}
		}
		let Pt = l(() => Object.keys(B).filter((e) => z[e] && !q(e)));
		function Ft(e) {
			I[e] = !I[e];
		}
		function It(e) {
		...
		async function Vt() {
			...
			for (; Date.now() - e < g.restartPollTimeoutMs;) {
				await Bt(t), t = Math.min(t + g.restartPollIntervalMs / 2, g.restartPollIntervalMs * 3);
				try {
					let e = await w.get();
					return E.value = e.types, O.value = e.meta, Et(e), F.value = {}, !0;
				} catch {}
			}
			return !1;
		}
		async function Ht() {
			if (!P.value) {
				P.value = !0;
				try {
					await w.restartServer(), T.success("Restart signal sent — waiting for the server to come back…"), await Vt() ? (_t(), T.success("Server is back online.")) : T.error(`The server did not respond within ${Math.round(g.restartPollTimeoutMs / 1e3)}s. It may still be starting — reload this page in a moment.`);
				} catch (e) {
		...
		async function Ut() {
			if (!N.value) {
				...
				try {
					...
					} else T.error(e instanceof i ? e.message : "Failed to save settings.");
				} finally {
					N.value = !1;
				}
			}
		}
		return fe(() => (mt(), Dt())), (e, t) => (_(), f("section", he, [
			...
			ot.value ? (_(), f("div", ge, [h(re, {
				...
				et.value ? (_(), f("div", _e, " This server did not send settings metadata, so help, grouping and Advanced tiers are unavailable. Every setting is listed below with a name derived from its key. Update the server to restore the full settings experience. ")) : d("", !0),
				p("div", ve, [h(oe, {
					modelValue: j.value,
					...
					"model-value": x(Ze).advancedMode,
					"onUpdate:modelValue": t[1] ||= (e) => x(Ze).setAdvancedMode(e)
				}, null, 8, ["model-value"])])]),
				dt.value ? (_(), f("div", be, [p("span", null, " Saved changes to " + b(pt.value.join(", ")) + " require a server restart to take effect. ", 1), p("span", xe, [p("button", {
					type: "button",
				...
				p("div", we, [ct.value.length === 0 ? (_(), f("p", Te, " No settings in this group. ")) : (_(), f("form", {
					key: 1,
					class: "admin-settings__form",
					onSubmit: me(Ut, ["prevent"])
				}, [(_(!0), f(c, null, pe(ct.value, (e) => (_(), f("div", {
					key: e,
					class: "admin-settings__field"
				}, [
					W(e) === "bool" ? (_(), f(c, { key: 0 }, [p("div", Ee, [
						h(te, {
						...
						text: Y(e) ?? "",
						links: X(e)
					}, null, 8, ["text", "links"])) : d("", !0)], 64)) : W(e) === "int" || W(e) === "float" ? (_(), f(c, { key: 1 }, [
						p("label", {
						...
						At(e).length ? (_(), f("dl", Ae, [(_(!0), f(c, null, pe(At(e), (e) => (_(), f(c, { key: e.value }, [p("dt", je, b(e.label), 1), p("dd", Me, b(e.help), 1)], 64))), 128))])) : d("", !0)
					], 64)) : vt(e) ? (_(), f(c, { key: 3 }, [
						p("label", {
							class: "admin-settings__label",
							for: `field-${e}`
						...
						p("p", {
							...
							_: 1
						}), t[16] ||= p("span", { class: "admin-settings__secret-hint" }, " The stored value will be deleted when you save. Undo to keep it. ", -1)], 64)) : U(e) === null ? (_(), f("span", Re, " This server did not report whether a value is stored. Type a new one to replace whatever is there; leave it blank to keep it. ")) : bt(e) ? (_(), f(c, { key: 2 }, [h(o, {
							tone: "accent",
							class: "admin-settings__secret-badge"
						}, {
							default: S(() => [...t[17] ||= [m("Configured", -1)]]),
							_: 1
						}), p("span", ze, " A value is stored (" + b(xt(e)) + " " + b(xt(e) === 1 ? "character" : "characters") + "). It is never sent to your browser. Leave this blank to keep it, or type a new one to replace it. ", 1)], 64)) : (_(), f(c, { key: 3 }, [h(o, {
							tone: "neutral",
							class: "admin-settings__secret-badge"
						}, {
						...
						p("div", Be, [
							...
							$(e) ? (_(), u(a, {
								...
								_: 1
							}, 8, ["aria-label", "onClick"])) : Lt(e) ? (_(), u(a, {
								key: 2,
								variant: "ghost",
								size: "sm",
					...
					ut(e) ? (_(), f("span", We, " Requires a server restart to take effect ")) : d("", !0),
...
//# sourceMappingURL=SettingsPage-CckEKvdZ.js.map