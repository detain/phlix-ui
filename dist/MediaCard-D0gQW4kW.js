...
import { n as ne, t as re } from "./ThumbRating-6vDNKJU1.js";
import { t as i } from "./Menu-DRkKveJV.js";
import { Fragment as a, computed as o, createBlock as ie, createCommentVNode as s, createElementBlock as c, createElementVNode as l, createTextVNode as u, createVNode as d, defineComponent as f, inject as p, normalizeClass as m, normalizeStyle as ae, onMounted as oe, openBlock as h, ref as g, renderList as se, renderSlot as _, toDisplayString as v, unref as y, withCtx as b, withModifiers as x } from "vue";
import { RouterLink as ce, routerKey as S } from "vue-router";
...
//#endregion
//#region src/utils/mediaTypeIcon.ts
var k = {
	movie: "film",
	series: "tv",
	season: "tv",
...
var ue = { class: "media-card__poster" }, de = [
...
}, Te = ["aria-label", "aria-pressed"], Ee = ["aria-label", "aria-pressed"], De = ["aria-expanded", "onClick"], Oe = { class: "media-card__caption" }, ke = ["title"], Ae = { class: "media-card__caption-sub numeric" }, z = /*#__PURE__*/ e(/* @__PURE__ */ f({
	__name: "MediaCard",
	props: {
		item: {},
		to: {},
	...
	setup(e, { emit: f }) {
		let w = e, T = f, E = ee(), D = r(), O = ne(), k = p("phlixConfig", null), z = p(S, null), B = o(() => O.isFavorite(w.item.id)), je = o(() => O.likeLevel(w.item.id)), V = o(() => D.isAdmin), H = o(() => O.isWatched(w.item.id)), U = g(!1), Me = o(() => w.item.type === "series" || w.item.type === "season"), Ne = o(() => (V.value, U.value ? M(w.item, {
			isAdmin: V.value,
			isWatched: H.value,
		...
		function Pe(e) {
			...
			switch (e.label) {
				...
				case t.markUnplayed:
					...
					break;
				case t.like:
					O.setLike(w.item.id, 1, k?.apiBase ?? "");
					break;
				case t.dislike:
					O.setLike(w.item.id, -1, k?.apiBase ?? "");
					break;
		...
		function Fe(e) {
			O.setLike(w.item.id, e, k?.apiBase ?? "");
		}
		function Ie() {
			O.toggleFavorite(w.item.id, k?.apiBase ?? ""), T("watchlist", w.item);
		}
		function W() {
			O.toggleWatched(w.item.id, k?.apiBase ?? ""), T("mark-watched", w.item);
		}
		...
		let Z = o(() => le(w.posterSrcset ?? w.item.poster_srcset, w.posterSizes)), Ge = o(() => {
		...
		return (n, r) => (h(), c("article", {
			...
			y(z) ? (h(), ie(y(ce), {
				...
				default: b(({ navigate: t }) => [l("a", {
					...
					onClick: t
				}, [l("span", fe, v(e.item.name), 1)], 8, de)]),
				_: 1
			}, 8, ["to"])) : (h(), c("a", {
				key: 1,
				href: G.value,
				class: "media-card__link",
				"aria-label": e.item.name
			}, [l("span", me, v(e.item.name), 1)], 8, pe)),
			e.item.poster_url ? (h(), c("img", {
				...
				onLoad: We
			}, null, 42, he)) : (h(), c("div", N, [d(t, { name: y(A)(e.item.type) }, null, 8, ["name"])])),
			l("div", P, [
				Ge.value ? (h(), c("span", F, "New")) : s("", !0),
				_(n.$slots, "badges", { item: e.item }, void 0, !0),
				e.quality ? (h(), c("span", I, v(e.quality), 1)) : s("", !0)
			]),
			Q.value > 0 ? (h(), c("div", {
				key: 4,
			...
			}, [l("i", { style: ae({ width: `${Q.value * 100}%` }) }, null, 4)], 8, L)) : s("", !0),
			l("div", R, [
				l("h3", ge, v(e.item.name), 1),
				l("div", _e, [
					e.item.year ? (h(), c("span", ve, v(e.item.year), 1)) : s("", !0),
					e.item.year && (e.item.rating || e.item.runtime) ? (h(), c("span", ye)) : s("", !0),
					e.item.rating ? (h(), c("span", be, v(e.item.rating), 1)) : s("", !0),
					e.item.rating && e.item.runtime ? (h(), c("span", xe)) : s("", !0),
					e.item.runtime ? (h(), c("span", Se, v(e.item.runtime) + "m", 1)) : s("", !0)
				]),
				$.value.length ? (h(), c("div", Ce, [(h(!0), c(a, null, se($.value, (e) => (h(), c("span", { key: e }, v(e), 1))), 128))])) : s("", !0),
				ze.value ? (h(), c("div", we, [l("button", {
					type: "button",
					...
					}, [d(t, { name: B.value ? "bookmark" : "bookmark-plus" }, null, 8, ["name"])], 10, Te),
					l("button", {
						type: "button",
					...
					}, [d(t, { name: H.value ? "eye" : "eye-off" }, null, 8, ["name"])], 10, Ee),
					l("button", {
						type: "button",
			...
			title: e.item.name
		}, v(e.item.name), 9, ke), l("div", Ae, [e.subtitle == null ? (h(), c(a, { key: 1 }, [
...
//#endregion
export { C as a, A as i, j as n, M as r, z as t };

//# sourceMappingURL=MediaCard-D0gQW4kW.js.map