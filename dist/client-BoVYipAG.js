...
function r(e, t = "Something went wrong.") {
...
var a = "access_token", o = "refresh_token", s = "user", c = class {
	storage;
	...
	setUser(e) {
		this.storage.setItem(s, JSON.stringify(e));
	}
	clear() {
		this.storage.removeItem(a), this.storage.removeItem(o), this.storage.removeItem(s);
	}
};
//#endregion
...
function v(e) {
	let t = e && typeof e == "object" ? e : {}, n = g(t.name) ?? "Unknown Artist", r = _(t.album_count), i = _(t.track_count);
	return {
		id: n,
		name: n,
...
function y(e) {
	...
	return {
		id: g(t.id) ?? "",
		title: r,
		durationSecs: _(n.duration_secs) ?? _(t.duration_secs) ?? 0,
		trackNumber: _(n.track_number) ?? _(t.track_number),
		streamUrl: g(t.stream_url)
	};
}
function b(e) {
	let t = e && typeof e == "object" ? e : {}, n = g(t.name) ?? g(t.title) ?? "Unknown Album", r = Array.isArray(t.tracks) ? t.tracks : [], i = _(t.track_count) ?? r.length;
	return {
		id: n,
		title: n,
		artist: g(t.artist),
		...
		totalTracks: i,
		tracks: r.map(y),
		tracksTruncated: h(t.tracks_truncated)
	};
}
var x = 100;
function S(e, t, n) {
	return {
		total: _(e.total) ?? t,
		limit: _(e.limit) ?? n.limit ?? 100,
		offset: _(e.offset) ?? n.offset ?? 0
	};
}
function C(e) {
	let t = {};
...
function T(t) {
	if (!(t instanceof e) || t.status !== 422) return !1;
	let n = t.body;
	return typeof n == "object" && !!n && n.code === "metadata.tmdb_unconfigured";
}
function E(e) {
	...
	return {
		...
		hearingImpaired: h(t.hearingImpaired ?? t.hearing_impaired),
		fps: a
	};
}
var D = class {
	...
	refreshPromise = null;
	constructor(e = {}) {
		this.baseUrl = e.baseUrl ?? (typeof window < "u" ? window.location.origin : ""), this.tokens = e.tokenStore ?? l(), this.doFetch = e.fetchImpl ?? globalThis.fetch.bind(globalThis), this.timeoutMs = e.timeoutMs ?? u, this.instanceHeaders = f(e.headers ?? {}), this.loginPath = e.loginPath ?? "/login";
	}
	setBaseUrl(e) {
		this.baseUrl = e;
	}
	async request(r, a, o = null, s) {
		let c = (e) => {
			let t = {
				...d,
				...this.instanceHeaders,
				"Content-Type": "application/json"
			}, n = this.tokens.getAccessToken();
			...
			return o !== null && (r === "POST" || r === "PUT" || r === "PATCH") && (i.body = JSON.stringify(o)), i;
		}, l = this.baseUrl !== "" && a.startsWith(this.baseUrl) ? a : `${this.baseUrl}${a}`, u = new AbortController(), f = !1, p = setTimeout(() => {
			f = !0, u.abort();
		}, this.timeoutMs), m = () => u.abort();
		s && (s.aborted ? u.abort() : s.addEventListener("abort", m, { once: !0 }));
		try {
			let e = await this.doFetch(l, c(u.signal));
			return e.status === 401 && await this.refreshToken() && (e = await this.doFetch(l, c(u.signal))), await this.handleResponse(e);
		} catch (r) {
			throw f ? new n() : s?.aborted || r instanceof e ? r : r instanceof TypeError || i() ? new t() : r;
		} finally {
			clearTimeout(p), s && s.removeEventListener("abort", m);
		}
	}
	async handleResponse(t) {
		if (t.status === 204 || t.status === 205) return;
		let n = (t.headers.get("content-type") ?? "").includes("application/json") ? await t.json() : await t.text();
		if (!t.ok) throw new e(this.extractError(n), t.status, n);
		return n;
	}
	extractError(e) {
	...
	async searchSubtitles(e, t, n) {
		...
		return Array.isArray(a.candidates) ? a.candidates.map(E) : [];
	}
	downloadSubtitle(e, t) {
		return this.post(`/api/v1/media/${encodeURIComponent(e)}/subtitles/download`, t);
	}
	async listArtists(e = {}, t) {
		let n = C(e), r = await this.get("/api/v1/music/artists", Object.keys(n).length ? n : void 0, t), i = Array.isArray(r.artists) ? r.artists : [];
		return {
			artists: i.map(v),
			...S(r, i.length, e)
		};
	...
	async getArtist(e, t) {
		return v((await this.get(`/api/v1/music/artist?name=${encodeURIComponent(e)}`, void 0, t)).artist);
	}
	...
	async getAlbum(e, t, n) {
		...
		return b((await this.get(`/api/v1/music/album?name=${encodeURIComponent(e)}${r}`, void 0, n)).album);
	}
	async listTracks(e = {}, t) {
		let n = C(e), r = await this.get("/api/v1/music/tracks", Object.keys(n).length ? n : void 0, t), i = Array.isArray(r.tracks) ? r.tracks : [];
		return {
			tracks: i.map(y),
			...S(r, i.length, e)
		};
	...
	async getTrack(e, t) {
		return y((await this.get(`/api/v1/music/tracks/${encodeURIComponent(e)}`, void 0, t)).track);
	}
	logout(e = !0) {
...
//#endregion
export { m as a, p as c, t as d, n as f, O as i, c as l, i as m, x as n, T as o, r as p, w as r, h as s, D as t, e as u };

//# sourceMappingURL=client-BoVYipAG.js.map