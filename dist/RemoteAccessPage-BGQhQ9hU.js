...
import { t as ne } from "./Modal-I4tEFhoH.js";
...
var fe = {
	...
	props: { client: {} },
	setup(e) {
		let ae = e, y = oe("apiBase", ""), rt = c(() => typeof y == "string" ? y : y?.value ?? ""), b = new ie(ae.client ?? new n({
			baseUrl: rt.value,
			tokenStore: new ee()
		})), x = re();
		function it(e) {
		...
		async function F() {
			...
			try {
				...
				E.value = i(e, "Failed to load hub status."), x.error(E.value);
			} finally {
				T.value = !1;
			}
		}
		...
		async function lt() {
			if (!N.value) {
				...
				try {
					...
					x.error(i(e, "Failed to initiate pairing."));
				} finally {
					N.value = !1;
				}
			}
		...
		async function ut() {
			if (!(M.value === null || A.value === "") && !P.value) {
				...
				try {
					...
					x.error(i(e, "Failed to poll pairing status."));
				} finally {
					P.value = !1;
				}
			}
		...
		async function dt() {
			if (!D.value) {
				...
				try {
					...
					x.error(i(e, "Failed to unenroll."));
				} finally {
					D.value = !1;
				}
			}
		...
		async function ft() {
			if (!O.value) {
				...
				try {
					...
					x.error(i(e, "Failed to send heartbeat."));
				} finally {
					O.value = !1;
				}
			}
		...
		async function V() {
			...
			try {
				...
				R.value = i(e, "Failed to load subdomain status."), x.error(R.value);
			} finally {
				L.value = !1;
			}
		}
		async function mt() {
			if (!z.value) {
				...
				try {
					...
					x.error(i(e, "Failed to claim subdomain."));
				} finally {
					z.value = !1;
				}
			}
		...
		async function ht() {
			if (!B.value) {
				...
				try {
					...
					x.error(i(e, "Failed to release subdomain."));
				} finally {
					B.value = !1;
				}
			}
		...
		async function vt() {
			...
			try {
				...
				W.value = i(e, "Failed to load relay status."), x.error(W.value);
			} finally {
				U.value = !1;
			}
		}
		async function yt() {
			if (!G.value) {
				...
				try {
					...
					x.error(i(e, "Failed to enable relay."));
				} finally {
					G.value = !1;
				}
			}
		...
		async function bt() {
			if (!K.value) {
				...
				try {
					...
					x.error(i(e, "Failed to disable relay."));
				} finally {
					K.value = !1;
				}
			}
		...
		async function xt() {
			if (!q.value) {
				...
				try {
					...
					x.error(i(e, "Failed to ping relay."));
				} finally {
					q.value = !1;
				}
			}
		...
		async function $() {
			...
			try {
				...
				X.value = i(e, "Failed to load port-forward status."), x.error(X.value);
			} finally {
				St.value = !1;
			}
		}
		async function Et() {
			if (!Z.value) {
				...
				try {
					...
					x.error(i(e, "Failed to enable port forwarding."));
				} finally {
					Z.value = !1;
				}
			}
		...
		async function Dt() {
			if (!Q.value) {
				...
				try {
					...
					x.error(i(e, "Failed to disable port forwarding."));
				} finally {
					Q.value = !1;
				}
			}
		...
		return se(() => {
			...
			])) : u("", !0), f("div", Ce, [w.value.paired ? (h(), d(s, { key: 1 }, [m(r, {
				variant: "outline",
				size: "sm",
				loading: O.value,
				onClick: ft
			...
			f("section", Ne, [f("button", {
				...
				_: 1
			}, 8, ["loading", "disabled"]), H.value.connected ? (h(), l(r, {
				key: 1,
				variant: "ghost",
				size: "sm",
				loading: K.value,
			...
			f("section", He, [f("button", {
				...
				f("div", Qe, [Y.value.enabled ? (h(), l(r, {
					key: 1,
					variant: "ghost",
					size: "sm",
					loading: Q.value,
...
//# sourceMappingURL=RemoteAccessPage-BGQhQ9hU.js.map