...
import { n as s, t as ne } from "./EmptyState-BEMIpc2l.js";
...
var ue = {
	...
	class: "admin-integrations__error",
	role: "alert"
}, Ke = 3e4, C = "openid profile email", w = /*#__PURE__*/ t(/* @__PURE__ */ h({
	...
	setup(t) {
		let h = {
			oidc: "OIDC",
			ldap: "LDAP"
		}, w = ["oidc", "ldap"], qe = t, T = ae("apiBase", ""), Je = ie(() => typeof T == "string" ? T : T?.value ?? ""), E = new re(qe.client ?? new n({
			baseUrl: Je.value,
			tokenStore: new ee()
		...
		async function P() {
			...
			try {
				...
				A.value = e(t, "Failed to load sync status."), D.error(A.value);
			} finally {
				k.value = !1;
			}
		}
		async function Ye() {
			...
			let t = !1;
			N(), M = setTimeout(() => {
				t = !0, j.value = !1, D.error("Sync timed out after 30 seconds. Check the server logs.");
			}, Ke);
			try {
				let e = await E.triggerSync();
				if (N(), t) return;
				e.success ? (D.success(e.message || "Sync complete."), await P()) : D.error(e.message || "Sync failed.");
			} catch (n) {
				if (N(), t) return;
				D.error(e(n, "Sync request failed."));
			} finally {
				t || (j.value = !1);
			}
		}
		async function Xe(t) {
			try {
				await E.setSyncEnabled(t), D.success(t ? "Auto-sync enabled." : "Auto-sync disabled."), await P();
			} catch (t) {
				D.error(e(t, "Failed to update sync setting."));
			}
		}
		...
		async function B() {
			...
			try {
				...
				L.value = e(t, "Failed to load auth providers."), D.error(L.value);
			} finally {
				I.value = !1;
			}
		}
		...
		async function Ze(t, n) {
			try {
				n ? (await E.disableProvider(t), D.success(`${h[t]} disabled.`)) : (await E.enableProvider(t), D.success(`${h[t]} enabled.`)), await B();
			} catch (n) {
				D.error(e(n, `Failed to update ${h[t]}.`));
			}
		}
		let H = _(!1), U = _({
			provider_url: "",
		...
		async function Qe() {
			...
			try {
				...
				D.error(e(t, "Failed to load OIDC settings."));
			}
			H.value = !0;
		}
		function $e() {
		...
		async function et() {
			...
			try {
				...
				G.value = e(t, "Failed to save OIDC settings.");
			} finally {
				W.value = !1;
			}
		}
		...
		async function tt() {
			...
			try {
				...
				D.error(e(t, "Failed to load LDAP settings."));
			}
			q.value = !0;
		}
		function nt() {
		...
		async function $() {
			...
			try {
				...
				Z.value = e(t, "Failed to save LDAP settings.");
			} finally {
				Y.value = !1;
			}
		}
		async function it() {
			...
			try {
				...
				D.error(e(t, "LDAP connection test failed."));
			} finally {
				X.value = !1;
			}
		}
...
//# sourceMappingURL=IntegrationsPage-BLKWEtPS.js.map