...
var fe = {
	...
	props: { client: {} },
	setup(e) {
		let ce = e, S = le("apiBase", ""), We = c(() => typeof S == "string" ? S : S?.value ?? ""), C = new se(ce.client ?? new t({
			baseUrl: We.value,
			tokenStore: new ee()
		})), w = re(), Ge = c(() => oe.map((e) => ({
			value: e.value,
		...
		let Z = g(null);
		async function ct() {
			let e = B.value, t = Z.value;
			if (!(!e || !t)) try {
				await C.removeProfile(t.id), w.success("Profile deleted."), Z.value = null, await W(e.id);
			} catch (e) {
		...
		async function ft() {
			...
			if (!(!e || !t)) {
				...
				lt.value = !0;
				try {
					await C.setPin(t.id, $.value), w.success("PIN set."), dt(), await W(e.id);
				} catch (e) {
		...
		async function pt(e) {
			let t = B.value;
			if (t) try {
				await C.clearPin(e.id), w.success("PIN cleared."), await W(t.id);
			} catch (e) {
...
//# sourceMappingURL=UsersPage-DVEFVwFt.js.map