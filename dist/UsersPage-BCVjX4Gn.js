...
import { d as t, n, s as ee, t as r } from "./Button-5ZSsUmsI.js";
...
var fe = {
	...
	props: { client: {} },
	setup(e) {
		let ce = e, S = le("apiBase", ""), Ye = c(() => typeof S == "string" ? S : S?.value ?? ""), C = new se(ce.client ?? new n({
			baseUrl: Ye.value,
			tokenStore: new ee()
		})), w = re(), Xe = c(() => oe.map((e) => ({
			value: e.value,
		...
		async function j(e) {
			try {
				...
				w.error(t(e, "Failed to approve user."));
			}
		}
		let M = g(null);
		async function tt() {
			...
			if (e) try {
				...
				w.error(t(e, "Failed to disable user.")), M.value = null;
			}
		}
		let N = g(null);
		async function nt() {
			...
			if (e) try {
				...
				w.error(t(e, "Failed to reject user.")), N.value = null;
			}
		}
		...
		async function lt() {
			...
			if (e) try {
				...
				w.error(t(e, "Failed to delete user.")), B.value = null;
			}
		}
		...
		async function dt(e) {
			...
			try {
				...
				w.error(t(e, "Failed to reset password.")), V.value = null;
			}
		}
		function ft() {
		...
		let Z = g(null);
		async function wt() {
			let e = U.value, n = Z.value;
			if (!(!e || !n)) try {
				await C.removeProfile(n.id), w.success("Profile deleted."), Z.value = null, await G(e.id);
			} catch (e) {
		...
		async function Ot() {
			...
			if (!(!e || !n)) {
				...
				Tt.value = !0;
				try {
					await C.setPin(n.id, $.value), w.success("PIN set."), Dt(), await G(e.id);
				} catch (e) {
		...
		async function kt(e) {
			let n = U.value;
			if (n) try {
				await C.clearPin(e.id), w.success("PIN cleared."), await G(n.id);
			} catch (e) {
...
//# sourceMappingURL=UsersPage-BCVjX4Gn.js.map