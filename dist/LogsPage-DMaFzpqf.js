...
import { t as s } from "./Select-Dl5D-rQ_.js";
...
var se = {
	...
	setup(e) {
		...
		function me(e) {
			let t = e.slice(0, 100);
			for (let [e, n] of Object.entries(D)) if (n.test(t)) return e;
			return null;
		}
		function he(e) {
			return e.replace(/\s+\[\]$/, "");
		...
		function A(e, t) {
			let n = e.level ? _e(e.level) : "", r = k(t || e.source), i = e.message;
			if (e.level) {
				let t = RegExp(`^${e.level.toUpperCase()}:\\s*`, "i");
				i = i.replace(t, "");
			}
			let a = k(i), o = a ? ge(a) : "", s = n ? ` ${n}` : "", c = e.localTime;
			return `${r ? `${r} ` : ""}${c}${s} ${o}`;
		}
		let j = e, M = re("apiBase", ""), N = f(() => typeof M == "string" ? M : M?.value ?? ""), P = new te(j.client ?? new r({
			...
			for (; n < e.length;) {
				...
				let i = [r], a = /* @__PURE__ */ new Set([r.source]), o = r.timestamp, s = n + 1;
				for (; s < e.length;) {
					let t = e[s];
...
}), [["__scopeId", "data-v-99d1307e"]]);
//#endregion
export { D as default };

//# sourceMappingURL=LogsPage-DMaFzpqf.js.map