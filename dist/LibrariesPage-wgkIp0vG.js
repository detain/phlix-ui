...
import { t as ee } from "./Select-CmN-4YbZ.js";
...
var ce = {
...
}, Se = { class: "admin-libraries__date" }, Ce = { class: "admin-libraries__date" }, we = 2e3, T = /*#__PURE__*/ t(/* @__PURE__ */ _({
	...
	setup(t) {
		let _ = t, T = ie("apiBase", ""), Te = d(() => typeof T == "string" ? T : T?.value ?? ""), E = new ne(_.client ?? new n({
			baseUrl: Te.value,
			tokenStore: new i()
		...
		async function z() {
			...
			try {
				...
				N.value = e(t, "Failed to load libraries."), D.error(N.value);
			} finally {
				M.value = !1;
			}
		}
		...
		async function J() {
			...
			try {
				let e = V.value;
				if (e) await E.update(e.id, {
					name: H.value,
					paths: t
				}), D.success("Library updated.");
				else {
					let e = await E.create({
						name: H.value,
						type: U.value,
						paths: t
					});
					D.success(e.message || "Library created.");
				}
				q(), await z();
			} catch (t) {
				D.error(e(t, "Failed to save library."));
			} finally {
				G.value = !1;
			}
		}
		...
		async function je() {
			let t = Y.value;
			if (t) try {
				await E.remove(t.id), D.success("Library deleted."), Y.value = null, await z();
			} catch (t) {
				D.error(e(t, "Failed to delete library.")), Y.value = null;
			}
		}
		async function X(t, n) {
			try {
				let e = n === "metadata" ? await E.matchMetadata(t.id) : n === "rescan" ? await E.rescan(t.id) : await E.scan(t.id), r = n === "metadata" ? `Metadata match queued (job ${e.job_id}).` : `Scan queued (job ${e.job_id}).`;
				D.success(e.message || r);
				let i = P.value[t.id];
				P.value = {
					...P.value,
					[t.id]: i ? {
				...
				D.error(e(t, "Failed to queue scan."));
			}
		}
		...
		async function Pe(t) {
			Z.value = t, Q.value = [], $.value = !0;
			try {
				Q.value = await E.scanHistory(t.id);
			} catch (t) {
				D.error(e(t, "Failed to load history."));
			} finally {
				$.value = !1;
			}
		}
...
//# sourceMappingURL=LibrariesPage-wgkIp0vG.js.map