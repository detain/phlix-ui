...
import { t as a } from "./useAuthStore-DWTuTW8p.js";
import { t as ee } from "./useToastStore-BDoKlU6N.js";
import { n as te, t as ne } from "./ThumbRating-DGyicxT5.js";
import { t as o } from "./Button-btm-GCUN.js";
import { t as s } from "./Chip-Dqypy8Bt.js";
import { a as re, n as ie, r as ae } from "./MediaCard-CJqS3fwx.js";
import { t as oe } from "./MediaRow-D0CkQKro.js";
import { Fragment as c, computed as l, createBlock as u, createCommentVNode as d, createElementBlock as f, createElementVNode as p, createTextVNode as m, createVNode as h, defineComponent as g, inject as se, normalizeClass as _, normalizeStyle as ce, onBeforeUnmount as le, onMounted as v, openBlock as y, ref as b, renderList as x, toDisplayString as S, watch as C, withCtx as w, withModifiers as ue } from "vue";
//#region src/components/MediaDetail.vue?vue&type=script&setup=true&lang.ts
var de = { class: "media-detail" }, fe = {
	key: 0,
	class: "media-detail__backdrop",
	"aria-hidden": "true"
}, pe = ["src", "srcset"], me = ["src"], he = { class: "media-detail__bar" }, ge = { class: "media-detail__hero" }, _e = { class: "media-detail__poster" }, ve = ["src", "alt"], ye = {
	key: 1,
	class: "media-detail__fallback",
	"aria-hidden": "true"
}, be = { class: "media-detail__info" }, xe = { class: "media-detail__title" }, Se = { class: "media-detail__meta numeric" }, Ce = {
	key: 0,
	...
	key: 2,
	class: "media-detail__meta-item"
}, Ee = { class: "media-detail__type" }, De = {
	key: 0,
	class: "media-detail__genres"
}, Oe = {
	key: 1,
	class: "media-detail__companies"
}, T = { class: "media-detail__company-list" }, ke = ["src", "alt"], Ae = { class: "media-detail__overview" }, je = { class: "media-detail__actions" }, Me = { class: "media-detail__resume-at numeric" }, Ne = {
	key: 1,
	...
	key: 1,
	class: "media-detail__credit-group"
}, Ye = { class: "media-detail__people" }, E = ["aria-label", "onClick"], Xe = { class: "media-detail__avatar" }, Ze = ["src", "alt"], Qe = {
	key: 1,
	class: "media-detail__avatar-initials",
	"aria-hidden": "true"
...
}, st = { class: "media-detail__file-size" }, D = "phlix.theme.muted", ct = .35, O = /*#__PURE__*/ e(/* @__PURE__ */ g({
	__name: "MediaDetail",
	props: {
		item: {},
	...
	setup(e, { emit: g }) {
		let O = e, k = g, A = te(), j = se("phlixConfig", null), lt = a(), M = l(() => A.isFavorite(O.item.id));
		function ut() {
			A.toggleFavorite(O.item.id, j?.apiBase ?? ""), k("watchlist", O.item);
		}
		let dt = l(() => A.likeLevel(O.item.id)), N = l(() => lt.isAdmin), P = l(() => A.isWatched(O.item.id));
		function F() {
		...
		let I = l(() => {
			...
			return r;
		}), L = b(!1), ft = l(() => O.item.type === "series" || O.item.type === "season"), pt = l(() => ae(O.item, {
			isAdmin: N.value,
			isWatched: P.value,
		...
		function mt(e) {
			...
			switch (e.label) {
				...
				case t.addToPlaylist: {
					...
					if (!e?.trim()) break;
					let t = e.trim();
					r.info("Creating playlist…"), n.createPlaylist(t, O.item.id).then(() => r.success("Playlist created")).catch((e) => {
						r.error("Failed to create playlist", { message: e instanceof Error ? e.message : String(e) });
					});
					break;
				}
				case t.download:
					r.info("Preparing download…"), n.getDownloadUrl(O.item.id).then(({ url: e }) => {
				...
				case t.missingEpisodes:
					r.info("Loading…"), n.getMissingEpisodes(O.item.id).then((e) => {
						e.length === 0 ? r.success("No missing episodes") : r.warning(`${e.length} episode${e.length === 1 ? "" : "s"} missing`);
					}).catch((e) => {
						r.error("Failed to load missing episodes", { message: e instanceof Error ? e.message : String(e) });
					});
					break;
				case t.shuffle:
					n.shufflePlay(O.item.id).then(() => r.success("Shuffle play started")).catch((e) => {
		...
		let gt = l(() => O.item.type === "audio" ? "music" : O.item.type === "image" ? "image" : O.item.type === "series" ? "tv" : "film"), R = l(() => {
			let e = O.item.cast;
		...
		let G = l(() => O.item.backdrop_url_large || O.item.backdrop_url || null), yt = l(() => O.item.backdrop_srcset || null), K = b(!1);
		function bt() {
			K.value = !0;
		}
		C(G, () => {
		...
		return v(() => {
			...
			G.value ? (y(), f("div", fe, [p("img", {
				...
				fetchpriority: "high",
				onLoad: bt
			}, null, 42, pe), r[8] ||= p("div", { class: "media-detail__backdrop-scrim" }, null, -1)])) : d("", !0),
			q.value ? (y(), f("audio", {
				key: 1,
			...
			e.item.poster_url ? (y(), f("div", {
				...
				style: ce({ backgroundImage: `url(${e.item.poster_url})` }),
				"aria-hidden": "true"
			}, null, 4)) : d("", !0),
			p("div", he, [e.showBack ? (y(), u(o, {
				key: 0,
				variant: "ghost",
				size: "sm",
			...
			p("div", ge, [p("div", _e, [e.item.poster_url ? (y(), f("img", {
				key: 0,
				ref_key: "imgEl",
				ref: W,
				...
				fetchpriority: "high",
				onLoad: vt
			}, null, 42, ve)) : (y(), f("div", ye, [h(t, { name: gt.value }, null, 8, ["name"])]))]), p("div", be, [
				p("h1", xe, S(e.item.name), 1),
				p("div", Se, [
					e.item.year ? (y(), f("span", Ce, [h(t, {
						name: "calendar",
						class: "media-detail__meta-icon"
					}), m(S(e.item.year), 1)])) : d("", !0),
					e.item.rating ? (y(), f("span", we, S(e.item.rating), 1)) : d("", !0),
					e.item.runtime ? (y(), f("span", Te, S(e.item.runtime) + "m", 1)) : d("", !0),
					p("span", Ee, S(e.item.type), 1)
				]),
				e.item.genres?.length ? (y(), f("div", De, [(y(!0), f(c, null, x(e.item.genres, (e) => (y(), u(s, {
					key: e,
					size: "sm",
				...
				B.value.length ? (y(), f("div", Oe, [r[10] ||= p("span", { class: "media-detail__companies-label" }, "Studios", -1), p("div", T, [(y(!0), f(c, null, x(B.value, (e) => (y(), u(s, {
					key: e.name,
					size: "sm",
				...
				p("div", je, [
					...
					q.value && !X.value ? (y(), f("div", Ne, [p("button", {
						...
						onClick: Dt
					}, [h(t, { name: Tt.value }, null, 8, ["name"])], 10, Pe), p("button", {
						type: "button",
				...
				], 8, He)]))), 128))])])) : d("", !0), R.value.length ? (y(), f("section", Je, [r[16] ||= p("h2", { class: "media-detail__credit-heading" }, "Cast", -1), p("ul", Ye, [(y(!0), f(c, null, x(R.value, (e, t) => (y(), f("li", { key: `cast-${t}-${e.name}` }, [p("button", {
					type: "button",
					class: "media-detail__person",
					"aria-label": `Show titles with ${e.name}`,
					onClick: (t) => k("actor", e.name)
			...
			e.similarLoading || e.similar.length ? (y(), u(oe, {
				key: 4,
...
}), [["__scopeId", "data-v-5dfe0098"]]);
//#endregion
export { O as t };

//# sourceMappingURL=MediaDetail-CSDFFNzb.js.map