...
}, A = { class: "admin-dlna__actions" }, j = /*#__PURE__*/ n(/* @__PURE__ */ v({
	__name: "DlnaServerPage",
	props: { client: {} },
	setup(n) {
		let v = n, j = y("apiBase", ""), M = d(() => typeof j == "string" ? j : j?.value ?? ""), N = new l(v.client ?? new a({
			baseUrl: M.value,
			tokenStore: new e()
		})), P = r(), F = S(null), I = S(!0), L = S(null), R = S(!1), z = d(() => F.value?.running ?? !1), B = d(() => F.value?.enabled ?? !1);
		async function V() {
			I.value = !0, L.value = null;
			try {
				F.value = await N.getStatus();
			} catch (e) {
				L.value = t(e, "Failed to load DLNA server status."), P.error(L.value);
			} finally {
				I.value = !1;
			}
		}
		async function H() {
			if (!R.value) {
				...
				try {
					...
					P.error(t(e, "Failed to start DLNA server."));
				} finally {
					R.value = !1;
				}
			}
		...
		async function U() {
			if (!R.value) {
				...
				try {
					...
					P.error(t(e, "Failed to stop DLNA server."));
				} finally {
					R.value = !1;
				}
			}
...
//# sourceMappingURL=DlnaServerPage--IQQJtTV.js.map