...
var pe = {
	...
	setup(e) {
		let _ = e, S = ae("apiBase", ""), Fe = u(() => typeof S == "string" ? S : S?.value ?? ""), C = new re(_.client ?? new n({
			baseUrl: Fe.value,
			tokenStore: new r()
		...
		async function z() {
			...
			try {
				...
				N.value = t(e, "Failed to load libraries."), w.error(N.value);
			} finally {
				M.value = !1;
			}
		}
		...
		async function We() {
			...
			try {
				...
				w.error(t(e, "Failed to save library."));
			} finally {
				K.value = !1;
			}
		}
		...
		async function Ge() {
			...
			if (e) try {
				...
				w.error(t(e, "Failed to delete library.")), J.value = null;
			}
		}
		async function Y(e, n) {
			try {
				let t = n === "metadata" ? await C.matchMetadata(e.id) : n === "rescan" ? await C.rescan(e.id) : await C.scan(e.id), r = n === "metadata" ? `Metadata match queued (job ${t.job_id}).` : `Scan queued (job ${t.job_id}).`;
				w.success(t.message || r);
				let i = P.value[e.id];
				P.value = {
					...P.value,
					[e.id]: i ? {
				...
				w.error(t(e, "Failed to queue scan."));
			}
		}
		...
		async function Je(e) {
			...
			try {
				...
				w.error(t(e, "Failed to load history."));
			} finally {
				Q.value = !1;
			}
		}
...
//# sourceMappingURL=LibrariesPage-C0A9z5qZ.js.map