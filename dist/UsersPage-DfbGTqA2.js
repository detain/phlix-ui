...
import { t as i } from "./Badge-D9Tdn6WP.js";
import { t as te } from "./Switch-R1pbcsd-.js";
import { t as ne } from "./Select-CmN-4YbZ.js";
import { t as a } from "./Modal-DLVWGUN9.js";
import { t as re } from "./useToastStore-BDoKlU6N.js";
import { n as ie, t as o } from "./EmptyState-BEMIpc2l.js";
...
var fe = {
	...
	key: 4,
	class: "admin-users__subform"
}, Be = { class: "admin-users__subform-title" }, Ve = { class: "admin-users__field" }, He = { class: "admin-users__subform-actions" }, Ue = 5, S = /*#__PURE__*/ t(/* @__PURE__ */ ce({
	__name: "UsersPage",
	props: { client: {} },
	setup(t) {
		let ce = t, S = le("apiBase", ""), We = c(() => typeof S == "string" ? S : S?.value ?? ""), C = new se(ce.client ?? new n({
			baseUrl: We.value,
			tokenStore: new ee()
		})), w = re(), Ge = c(() => oe.map((e) => ({
			value: e.value,
		...
		async function O() {
			...
			try {
				...
				D.value = e(t, "Failed to load users."), w.error(D.value);
			} finally {
				E.value = !1;
			}
		}
		...
		async function Ye() {
			...
			try {
				if (t) {
					...
					N.value && (e.password = N.value), await C.update(t.id, e);
					let n = +!!P.value;
					t.is_admin !== n && await C.setAdmin(t.id, P.value), w.success("User updated.");
				} else {
					let e = {
						username: j.value,
						email: M.value,
				...
				w.error(e(t, "Failed to save user."));
			} finally {
				F.value = !1;
			}
		}
		...
		async function Xe() {
			let t = L.value;
			if (t) try {
				await C.remove(t.id), w.success("User deleted."), L.value = null, await O();
			} catch (t) {
				w.error(e(t, "Failed to delete user.")), L.value = null;
			}
		}
		async function Ze(t, n) {
			try {
				await C.setAdmin(t.id, n), w.success(n ? "User promoted to admin." : "Admin status removed."), await O();
			} catch (t) {
				w.error(e(t, "Failed to update admin status."));
			}
		}
		let R = g(null), z = g(null);
		async function Qe(t) {
			R.value = t, z.value = null;
			try {
				z.value = await C.resetPassword(t.id);
			} catch (t) {
				w.error(e(t, "Failed to reset password.")), R.value = null;
			}
		}
		function $e() {
		...
		async function W(t) {
			H.value = !0;
			try {
				V.value = await C.listProfiles(t);
			} catch (t) {
				w.error(e(t, "Failed to load profiles."));
			} finally {
				H.value = !1;
			}
		}
			...
			if (t) {
				...
				try {
					if (K.value) {
						...
						await C.createProfile(t.id, e), w.success("Profile created.");
					}
					X(), await W(t.id);
				} catch (t) {
					w.error(e(t, "Failed to save profile."));
				} finally {
					Y.value = !1;
				}
			}
		...
		let Z = g(null);
		async function ct() {
			let t = B.value, n = Z.value;
			if (!(!t || !n)) try {
				await C.removeProfile(n.id), w.success("Profile deleted."), Z.value = null, await W(t.id);
			} catch (t) {
				w.error(e(t, "Failed to delete profile.")), Z.value = null;
			}
		}
		...
		async function ft() {
			...
			if (!(!t || !n)) {
				...
				lt.value = !0;
				try {
					await C.setPin(n.id, $.value), w.success("PIN set."), dt(), await W(t.id);
				} catch (t) {
					w.error(e(t, "Failed to set PIN."));
				} finally {
					lt.value = !1;
				}
			}
		...
		async function pt(t) {
			let n = B.value;
			if (n) try {
				await C.clearPin(t.id), w.success("PIN cleared."), await W(n.id);
			} catch (t) {
				w.error(e(t, "Failed to clear PIN."));
			}
		}
		function mt(e) {
			return ae[e] ?? ae[6];
...
//# sourceMappingURL=UsersPage-DfbGTqA2.js.map