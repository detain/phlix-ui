...
import { a as ee } from "./plural-DMM7pLFA.js";
import { l as n, p as r, t as te, u as ne } from "./client-COHWZ2KC.js";
...
import { t as ae } from "./Modal-DRyCYCuK.js";
import { t as o } from "./Skeleton-jlFj-j5t.js";
import { t as s } from "./EmptyState-DERkIIRd.js";
import { t as oe } from "./PageHint-DVe81aMu.js";
import { t as se } from "./remoteAccess-DVKRpKQ8.js";
import { t as ce } from "./helpLinks-BI4oN4Or.js";
import { Fragment as c, computed as l, createBlock as u, createCommentVNode as d, createElementBlock as f, createElementVNode as p, createStaticVNode as le, createTextVNode as m, createVNode as h, defineComponent as ue, inject as de, normalizeClass as fe, normalizeStyle as pe, onMounted as me, openBlock as g, ref as _, renderList as he, toDisplayString as v, unref as y, vModelText as ge, withCtx as b, withDirectives as _e, withModifiers as ve } from "vue";
...
}, Pt = { class: "admin-remote__claim-code" }, Ft = { class: "admin-remote__field" }, It = { class: "admin-remote__field" }, Lt = 10, Rt = /*#__PURE__*/ e(/* @__PURE__ */ ue({
	__name: "RemoteAccessPage",
	props: { client: {} },
	setup(e) {
		let ue = e, Rt = de("apiBase", ""), zt = l(() => typeof Rt == "string" ? Rt : Rt?.value ?? ""), x = new se(ue.client ?? new te({
			baseUrl: zt.value,
			tokenStore: new n()
		})), Bt = new ie(ue.client ?? new te({
			baseUrl: zt.value,
			tokenStore: new n()
		})), S = re();
		function C(e) {
		...
		function T(e) {
			let t = w.value[e];
			w.value[e] = !t, e === "networkhealth" && !t && xn();
		}
		let E = _(null), Vt = _(!0), D = _(null), Ht = _(!1), O = _(!1), Ut = _(!1), k = _(""), Wt = _("Phlix Server"), A = _(null), Gt = _(null), Kt = _(!1), qt = _(!1), Jt = l(() => Vt.value ? "Loading…" : E.value === null ? "Unable to load" : E.value.paired ? `Paired${E.value.serverId ? ` (${E.value.serverId})` : ""}` : "Not paired");
		async function Yt() {
			Vt.value = !0, D.value = null;
			try {
		...
		function Zt() {
			Ut.value = !1, A.value = null, Gt.value = null;
		}
		async function Qt() {
			if (!Kt.value) {
				if (k.value === "") {
				...
				Kt.value = !0;
				try {
					let e = await x.hubPair(k.value, Wt.value);
					e.success && (A.value = e.claimCode ?? null, Gt.value = e.claimId ?? null, S.success("Pairing initiated. Complete the claim on the hub, then poll."));
				} catch (e) {
		...
		async function $t() {
			if (Gt.value !== null && k.value !== "" && !qt.value) {
				qt.value = !0;
				try {
					let e = await x.hubPoll(Gt.value, k.value);
					e.success && e.token ? (await x.hubComplete(e.token, "", e.serverId ?? "", k.value), S.success("Hub paired successfully."), Zt(), await Yt()) : !e.success && e.message && S.error(e.message);
				} catch (e) {
		...
		async function en() {
			if (!Ht.value) {
				Ht.value = !0;
				try {
					await x.hubUnenroll(), S.success("Hub unenrolled."), await Yt();
				} catch (e) {
		...
		let j = _(null), M = _(!0), N = _(null), P = _(!1), F = _(!1), nn = l(() => M.value ? "Loading…" : j.value === null ? "Unable to load" : j.value.claimed ? `Claimed${j.value.subdomain ? ` (${j.value.subdomain})` : ""}` : "Not claimed");
		async function I() {
			M.value = !0, N.value = null;
			try {
				j.value = await x.subdomainStatus();
			} catch (e) {
				N.value = r(e, "Failed to load subdomain status."), S.error(N.value);
			} finally {
				M.value = !1;
			}
		}
		async function rn() {
			if (!P.value) {
				P.value = !0;
				try {
					await x.subdomainClaim(), S.success("Subdomain claimed."), await I();
				} catch (e) {
		...
		async function an() {
			if (!F.value) {
				F.value = !0;
				try {
					await x.subdomainRelease(), S.success("Subdomain released."), await I();
				} catch (e) {
		...
		let L = _(null), R = _(!0), z = _(null), B = _(!1), V = _(!1), on = _(!1), H = _(null), sn = l(() => {
		...
		async function U() {
			...
			try {
				L.value = await x.relayStatus(), H.value = null;
			} catch (e) {
		...
		async function un() {
			if (!B.value) {
				...
				try {
					...
					S.success(e.message || "Relay enabled; takes effect on the next server reload.", e.disabled ? { tone: "warning" } : void 0), await U();
				} catch (e) {
		...
		async function dn() {
			if (!V.value) {
				...
				try {
					...
					S.success(e.message || "Relay disabled; takes effect on the next server reload."), await U();
				} catch (e) {
		...
		async function fn() {
			if (!on.value) {
				...
				try {
					...
					H.value = e, e.latencyMs == null ? S.info("Relay connected; latency not measured yet.") : S.success(`Relay latency: ${e.latencyMs}ms`);
				} catch (e) {
					if (H.value = null, e instanceof ne && e.status === 409) {
						let t = e.body ?? null, ee = t?.lastConnectError ? ` (${t.lastConnectError})` : "";
						S.error(`${t?.message ?? "Relay not connected."}${ee}`), await U();
		...
		let W = _(null), pn = _(!0), G = _(null), K = _(!1), q = _(!1), mn = _([]), hn = l(() => pn.value ? "Loading…" : W.value === null ? "Unable to load" : W.value.enabled ? W.value.externalIp ? `Enabled (${W.value.externalIp}:${W.value.externalPort})` : "Enabled" : "Disabled"), gn = l(() => K.value || q.value);
		async function _n() {
			pn.value = !0, G.value = null;
			try {
				let [e, t] = await Promise.all([x.portForwardStatus(), x.portForwardCandidates()]);
				W.value = e, mn.value = t.candidates;
			} catch (e) {
		...
		async function vn() {
			if (!K.value) {
				K.value = !0;
				try {
					await x.portForwardEnable(), S.success("Port forwarding enabled."), await _n();
				} catch (e) {
		...
		async function yn() {
			if (!q.value) {
				q.value = !0;
				try {
					await x.portForwardDisable(), S.success("Port forwarding disabled."), await _n();
				} catch (e) {
		...
		return me(() => {
			...
			h(oe, {
				links: y(ce)["remote-access"].links,
				details: y(ce)["remote-access"].details
			}, {
				default: b(() => [...n[9] ||= [
			...
			p("section", Se, [p("button", {
			...
			}, null, 8, ["name"])]), p("span", Te, v(Jt.value), 1)], 8, Ce), w.value.hub ? (g(), f("div", Ee, [Vt.value ? (g(), f("div", De, [h(o, {
				variant: "text",
				lines: 3
			})])) : D.value ? (g(), u(s, {
				key: 1,
				...
				default: b(() => [...n[16] ||= [m(" Send Heartbeat ", -1)]]),
				_: 1
			}, 8, ["loading"]), h(i, {
				variant: "ghost",
				size: "sm",
				...
				default: b(() => [...n[15] ||= [m(" Initiate Pairing ", -1)]]),
				_: 1
			}))])], 64))])) : d("", !0)]),
			p("section", je, [p("button", {
				...
				class: "admin-remote__chevron"
			}, null, 8, ["name"])]), p("span", Pe, v(nn.value), 1)], 8, Me), w.value.subdomain ? (g(), f("div", Fe, [M.value ? (g(), f("div", Ie, [h(o, {
				variant: "text",
				lines: 2
			})])) : N.value ? (g(), u(s, {
				key: 1,
				...
				default: b(() => [...n[23] ||= [m(" Release Subdomain ", -1)]]),
				_: 1
			}, 8, ["loading"])) : (g(), u(i, {
				key: 0,
				variant: "solid",
				...
				default: b(() => [...n[22] ||= [m(" Claim Subdomain ", -1)]]),
				_: 1
			}, 8, ["loading"]))])], 64))])) : d("", !0)]),
			p("section", Be, [p("button", {
				...
				class: "admin-remote__chevron"
			}, null, 8, ["name"])]), p("span", Ue, v(sn.value), 1)], 8, Ve), w.value.relay ? (g(), f("div", We, [R.value ? (g(), f("div", Ge, [h(o, {
				variant: "text",
				lines: 2
			})])) : z.value ? (g(), u(s, {
				key: 1,
				...
				p("dl", qe, [
					...
					p("dd", null, [h(a, { tone: L.value.connected ? "success" : "neutral" }, {
						default: b(() => [m(v(L.value.connected ? "Connected" : "Disconnected"), 1)]),
						_: 1
					}, 8, ["tone"])]),
					...
					p("dd", null, [h(a, { tone: L.value.enrolled ? "success" : "neutral" }, {
						default: b(() => [m(v(L.value.enrolled ? "Yes" : "No"), 1)]),
						_: 1
					}, 8, ["tone"])]),
					n[31] ||= p("dt", null, "Kill-switch", -1),
					p("dd", null, [h(a, { tone: L.value.disabled ? "warning" : "success" }, {
						default: b(() => [m(v(L.value.disabled ? "Disabled" : "Enabled"), 1)]),
						_: 1
					}, 8, ["tone"])]),
					ln.value === null ? d("", !0) : (g(), f(c, { key: 0 }, [n[26] ||= p("dt", null, "Latency", -1), p("dd", null, [m(v(ln.value) + " ", 1), H.value && H.value.latencyMs != null ? (g(), f("span", Je, " (last recorded heartbeat) ")) : d("", !0)])], 64)),
					L.value.lastConnectError ? (g(), f(c, { key: 1 }, [n[27] ||= p("dt", null, "Last error", -1), p("dd", Ye, [m(v(L.value.lastConnectError) + " ", 1), L.value.lastConnectErrorAt ? (g(), f(c, { key: 0 }, [m(" (" + v(C(L.value.lastConnectErrorAt)) + ") ", 1)], 64)) : d("", !0)])], 64)) : d("", !0)
				]),
				n[35] ||= p("p", {
			...
			p("section", Ze, [p("button", {
				...
				class: "admin-remote__chevron"
			}, null, 8, ["name"])]), p("span", et, v(hn.value), 1)], 8, Qe), w.value.portforward ? (g(), f("div", tt, [pn.value ? (g(), f("div", nt, [h(o, {
				variant: "text",
				lines: 3
			})])) : G.value ? (g(), u(s, {
				key: 1,
				...
				p("dl", it, [
					...
					p("dd", null, [h(a, { tone: W.value.enabled ? "success" : "neutral" }, {
						default: b(() => [m(v(W.value.enabled ? "Yes" : "No"), 1)]),
						_: 1
					}, 8, ["tone"])]),
				...
				mn.value.length > 0 ? (g(), f("div", at, [n[42] ||= p("h3", { class: "admin-remote__candidates-title" }, "Hostname Candidates", -1), p("ul", ot, [(g(!0), f(c, null, he(mn.value, (e, t) => (g(), f("li", { key: t }, v(e.hostname), 1))), 128))])])) : d("", !0),
			...
			p("section", ct, [p("button", {
				...
				class: "admin-remote__chevron"
			}, null, 8, ["name"])]), p("span", dt, v(Sn.value), 1)], 8, lt), w.value.networkhealth ? (g(), f("div", ft, [Q.value ? (g(), f("div", pt, [h(o, {
				variant: "text",
				lines: 4
			})])) : $.value ? (g(), u(s, {
				key: 1,
				...
				p("div", mt, [
					p("div", ht, [n[51] ||= p("h3", { class: "admin-remote__health-card-title" }, "Relay Tunnel", -1), p("dl", gt, [
						...
						p("dd", null, [h(a, { tone: J.value.connected ? "success" : "error" }, {
							default: b(() => [m(v(J.value.connected ? J.value.active ? "Active" : "Connecting" : "Disconnected"), 1)]),
							_: 1
						}, 8, ["tone"])]),
					...
					p("div", yt, [n[60] ||= p("h3", { class: "admin-remote__health-card-title" }, "Network Latency", -1), p("dl", bt, [
						...
						p("dd", null, [h(a, { tone: X.value?.status === "healthy" ? "success" : X.value?.status === "degraded" ? "warning" : "error" }, {
							default: b(() => [m(v(X.value?.latencyMs == null ? "N/A" : `${X.value?.latencyMs}ms`), 1)]),
							_: 1
						}, 8, ["tone"])]),
				...
				Z.value.length > 0 ? (g(), f("div", St, [
					p("h3", Ct, "Latency History (last " + v(y(ee)(Z.value.length, "measurement", "measurements")) + ")", 1),
					p("div", {
						class: "admin-remote__latency-bars",
						role: "img",
						"aria-label": `Latency graph showing ${y(ee)(Z.value.length, "measurement", "measurements")}`
					}, [(g(!0), f(c, null, he(Z.value, (e, t) => (g(), f("div", {
						key: t,
						class: "admin-remote__latency-bar-wrap",
						title: `${e.ms}ms at ${C(e.at)}`
					}, [p("div", {
						class: fe(["admin-remote__latency-bar", `admin-remote__latency-bar--${e.ms < 100 ? "good" : e.ms < 500 ? "warn" : "bad"}`]),
						style: pe({ height: `${Math.min(100, e.ms / 600 * 100)}%` })
					}, null, 6), p("span", Et, v(e.ms), 1)], 8, Tt))), 128))], 8, wt),
					n[61] ||= le("<div class=\"admin-remote__latency-legend\" data-v-55dce9ca><span class=\"admin-remote__latency-legend-item\" data-v-55dce9ca><span class=\"admin-remote__latency-dot admin-remote__latency-dot--good\" data-v-55dce9ca></span> &lt;100ms</span><span class=\"admin-remote__latency-legend-item\" data-v-55dce9ca><span class=\"admin-remote__latency-dot admin-remote__latency-dot--warn\" data-v-55dce9ca></span> 100-500ms</span><span class=\"admin-remote__latency-legend-item\" data-v-55dce9ca><span class=\"admin-remote__latency-dot admin-remote__latency-dot--bad\" data-v-55dce9ca></span> &gt;500ms</span></div>", 1)
				])) : d("", !0),
				p("div", Dt, [h(i, {
					variant: "outline",
					size: "sm",
			...
			h(ae, {
				...
				footer: b(() => [h(i, {
					...
					default: b(() => [...n[67] ||= [m(" Poll for Completion ", -1)]]),
					_: 1
				}, 8, ["loading"])) : (g(), u(i, {
					key: 1,
					variant: "solid",
					...
					default: b(() => [...n[68] ||= [m(" Initiate Pairing ", -1)]]),
					_: 1
				}, 8, ["loading", "disabled"]))]),
				default: b(() => [A.value ? (g(), f("div", Nt, [n[63] ||= p("p", null, "Enter this claim code on the hub:", -1), p("p", Pt, v(A.value), 1)])) : (g(), f("form", {
					key: 1,
					class: "admin-remote__form",
					onSubmit: ve(Qt, ["prevent"])
					...
					required: ""
				}, null, 512), [[ge, k.value]])]), p("label", It, [n[65] ||= p("span", { class: "admin-remote__label" }, "Server name", -1), _e(p("input", {
...
}), [["__scopeId", "data-v-55dce9ca"]]);
//#endregion
export { Rt as default };

//# sourceMappingURL=RemoteAccessPage-nFPXtAk9.js.map