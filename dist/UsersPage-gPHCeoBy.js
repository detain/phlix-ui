...
import { l as t, p as n, t as r } from "./client-DA-5QZXw.js";
import { a as ee, n as te, r as i, s as ne, t as re } from "./users-BVx6liqw.js";
import { t as ie } from "./useToastStore-BDoKlU6N.js";
import { t as a } from "./Button-BL3fV7FU.js";
import { t as o } from "./Badge-DbdgvC-x.js";
import { t as ae } from "./Switch-DyS2L5gX.js";
import { t as oe } from "./Select-B_7Gkn8F.js";
import { t as s } from "./Modal-B5BQA5yV.js";
import { t as se } from "./Skeleton-jlFj-j5t.js";
import { t as ce } from "./EmptyState-BwwPJtFd.js";
import { t as le } from "./PageHint-CJN_ODn2.js";
...
}, tt = { class: "admin-users__relay-section" }, nt = { class: "admin-users__field" }, rt = { class: "admin-users__relay-section" }, it = { class: "admin-users__relay-note" }, at = { class: "admin-users__field" }, ot = { class: "admin-users__field" }, st = { class: "admin-users__field" }, ct = 5, lt = 1024 ** 3, ut = 0x4000000000000, dt = 1e3, ft = /*#__PURE__*/ e(/* @__PURE__ */ de({
	__name: "UsersPage",
	props: { client: {} },
	setup(e) {
		let de = e, ft = fe("apiBase", ""), pt = l(() => typeof ft == "string" ? ft : ft?.value ?? ""), S = new re(de.client ?? new r({
			baseUrl: pt.value,
			tokenStore: new t()
		})), C = ie(), mt = fe("phlixConfig", null), ht = l(() => mt?.app === "hub"), gt = l(() => ee.map((e) => ({
			value: e.value,
			label: e.label
		}))), w = _([]), _t = _(!0), T = _(null);
		async function E() {
			...
			try {
				...
				T.value = n(e, "Failed to load users."), C.error(T.value);
			} finally {
				_t.value = !1;
			}
		}
		...
		let vt = l(() => w.value.filter((e) => D(e) === "pending")), yt = {
			pending: "Pending",
			active: "Active",
			disabled: "Disabled"
		}, bt = {
		...
		async function Ct(e) {
			try {
				await S.approve(e.id), C.success(`${e.username} approved.`), await E();
			} catch (e) {
		...
		async function wt() {
			let e = O.value;
			if (e) try {
				await S.disable(e.id), C.success(`${e.username} disabled.`), O.value = null, await E();
			} catch (e) {
				C.error(n(e, "Failed to disable user.")), O.value = null;
			}
		}
		let k = _(null);
		async function Tt() {
			let e = k.value;
			if (e) try {
				await S.reject(e.id), C.success(`${e.username}'s signup rejected.`), k.value = null, await E();
			} catch (e) {
				C.error(n(e, "Failed to reject user.")), k.value = null;
			}
		}
		...
		async function jt() {
			...
			try {
				if (e) {
					...
					P.value && (t.password = P.value), await S.update(e.id, t), e.is_admin !== F.value && await S.setAdmin(e.id, F.value), C.success("User updated.");
				} else {
					let e = {
						username: M.value,
						email: N.value,
		...
		async function Mt() {
			let e = I.value;
			if (e) try {
				await S.remove(e.id), C.success("User deleted."), I.value = null, await E();
			} catch (e) {
				C.error(n(e, "Failed to delete user.")), I.value = null;
			}
		}
		async function Nt(e, t) {
			try {
				await S.setAdmin(e.id, t), C.success(t ? "User promoted to admin." : "Admin status removed."), await E();
			} catch (e) {
		...
		async function Pt(e) {
			...
			try {
				...
				C.error(n(e, "Failed to reset password.")), L.value = null;
			}
		}
		function Ft() {
		...
		async function Yt() {
			...
			if (!(!e || !t)) try {
				await S.removeProfile(t.id), C.success("Profile deleted."), G.value = null, await V(e.id);
			} catch (e) {
		...
		async function $t() {
			...
			if (!(!e || !t)) {
				...
				Xt.value = !0;
				try {
					await S.setPin(t.id, q.value), C.success("PIN set."), Qt(), await V(e.id);
				} catch (e) {
		...
		async function en(e) {
			let t = z.value;
			if (t) try {
				await S.clearPin(e.id), C.success("PIN cleared."), await V(t.id);
			} catch (e) {
		...
		let tn = l(() => ne.map((e) => ({
			value: e.value,
			label: e.label
		}))), J = _(null), nn = _(!1), rn = _(!1), Y = _(null), X = _(te), Z = _("0"), Q = _("0"), $ = _("0"), an = _({
			quotaInGiB: "0",
			quotaOutGiB: "0",
			maxStreams: "0"
		...
		async function mn() {
			...
			let i = an.value, ne = X.value !== t.throttle_bps, re = String(Z.value) !== i.quotaInGiB || String(Q.value) !== i.quotaOutGiB || String($.value) !== i.maxStreams;
			if (!ne && !re) {
			...
			rn.value = !0;
			try {
				if (ne && await S.setThrottle(e.id, X.value), re) {
					let t = {
						quota_bytes_in: r,
						quota_bytes_out: ee,
		...
		return pe(E), (e, t) => (g(), f("section", _e, [
			...
			_t.value ? (g(), f("div", ye, [h(se, {
				...
				_: 1
			}, 8, ["description"])) : w.value.length === 0 ? (g(), u(ce, {
				key: 2,
				icon: "user",
				title: "No users yet"
			}, {
				...
				_: 1
			})) : (g(), f(c, { key: 3 }, [vt.value.length > 0 ? (g(), f("section", be, [p("h2", xe, [t[26] ||= m(" Pending approval ", -1), h(o, { tone: "warning" }, {
			...
			])], -1), p("tbody", null, [(g(!0), f(c, null, me(w.value, (e) => (g(), f("tr", { key: e.id }, [
				p("td", null, v(e.username), 1),
				p("td", null, v(e.email), 1),
				p("td", null, [h(o, { tone: e.is_admin ? "accent" : "neutral" }, {
					default: b(() => [m(v(e.is_admin ? "Admin" : "User"), 1)]),
					_: 2
				}, 1032, ["tone"])]),
				p("td", null, [h(o, { tone: St(e) }, {
				...
				p("td", null, [p("div", De, [
					D(e) === "pending" ? (g(), u(a, {
						...
						_: 1
					}, 8, ["aria-label", "onClick"])) : D(e) === "disabled" ? (g(), u(a, {
						key: 1,
						variant: "ghost",
						size: "sm",
			...
			h(s, {
				...
				default: b(() => [Lt.value ? (g(), f("div", Re, [h(se, {
					variant: "text",
					lines: 4
				})])) : (g(), f(c, { key: 1 }, [
					p("div", ze, [h(a, {
					...
					B.value.length === 0 ? (g(), u(ce, {
						...
						p("td", null, [h(o, { tone: e.pin_hash === null ? "neutral" : "success" }, {
					...
					Ut.value ? (g(), f("div", He, [p("h3", Ue, v(H.value ? "Edit profile" : "Add profile"), 1), p("form", {
						...
						p("label", Ge, [t[66] ||= p("span", { class: "admin-users__label" }, "Rating", -1), h(oe, {
							"model-value": W.value,
							options: gt.value,
							label: "Rating",
					...
					K.value ? (g(), f("div", Ye, [p("h3", Xe, "Set PIN — " + v(K.value.name), 1), p("form", {
						...
						required: ""
					}, null, 512), [[y, q.value]])]), p("div", Qe, [h(a, {
						variant: "ghost",
						size: "sm",
						onClick: Qt
					}, {
			...
			h(s, {
				...
				default: b(() => [nn.value ? (g(), f("div", $e, [h(se, {
					variant: "text",
					lines: 4
					...
					p("label", nt, [t[75] ||= p("span", { class: "admin-users__label" }, "Throttle", -1), h(oe, {
						"model-value": X.value,
						options: tn.value,
						label: "Throttle",
...
//# sourceMappingURL=UsersPage-gPHCeoBy.js.map