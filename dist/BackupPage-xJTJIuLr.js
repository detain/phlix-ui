...
var oe = { class: "admin-backup" }, se = {
	...
	key: 1,
	class: "admin-backup__muted"
}, Ce = { class: "admin-backup__form-row" }, we = { class: "admin-backup__field" }, Te = { class: "admin-backup__field" }, Ee = { class: "admin-backup__form-actions" }, De = { class: "admin-backup__field" }, T = /*#__PURE__*/ t(/* @__PURE__ */ g({
	__name: "BackupPage",
	props: { client: {} },
	setup(t) {
		let g = t, T = re("apiBase", ""), E = ne(() => typeof T == "string" ? T : T?.value ?? ""), D = new te(g.client ?? new n({
			baseUrl: E.value,
			tokenStore: new i()
		})), O = ee();
		function Oe(e) {
		...
		async function N() {
			...
			try {
				...
				j.value = e(t, "Failed to load backups."), O.error(j.value);
			} finally {
				A.value = !1;
			}
		}
		...
		async function z() {
			...
			try {
				...
				O.error(e(t, "Failed to create backup."));
			} finally {
				I.value = !1;
			}
		}
		...
		async function je() {
			...
			if (t) {
				V.value = !0;
				try {
					let e = await D.restore(t.id);
					O.success(e.message || "Restore completed."), H();
				} catch (t) {
					O.error(e(t, "Restore failed.")), H();
				}
			}
		}
		let U = v(null), W = v(!1);
		...
		async function Me() {
			...
			if (t) {
				W.value = !0;
				try {
					await D.delete(t.id), O.success("Backup deleted."), G(), await N();
				} catch (t) {
					O.error(e(t, "Failed to delete backup.")), G();
				}
			}
		}
		async function Ne(t) {
			M.value = t.id;
			try {
				let e = await D.uploadToS3(t.id);
				O.success(e.message || "Uploaded to S3."), await N();
			} catch (t) {
				O.error(e(t, "S3 upload failed."));
			} finally {
				M.value = null;
			}
		}
		...
		async function Q() {
			...
			try {
				...
				J.value = e(t, "Failed to load schedule."), O.error(J.value);
			} finally {
				q.value = !1;
			}
		}
		async function $() {
			let t = parseInt(Y.value, 10), n = parseInt(X.value, 10);
			if (isNaN(t) || t < 0) {
			...
			Z.value = !0;
			try {
				let e = await D.updateSchedule({
					auto_backup_interval_days: t,
					retention_count: n
				});
				O.success("Schedule saved."), K.value &&= {
					...K.value,
					auto_backup_interval_days: e.auto_backup_interval_days,
					retention_count: e.retention_count
				};
			} catch (t) {
				O.error(e(t, "Failed to save schedule."));
			} finally {
				Z.value = !1;
			}
		}
...
//# sourceMappingURL=BackupPage-xJTJIuLr.js.map