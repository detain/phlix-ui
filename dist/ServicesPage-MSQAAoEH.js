...
var E = {
...
}, O = { class: "admin-services__section-head" }, k = { class: "admin-services__card" }, A = {
	key: 0,
...
}, N = { class: "admin-services__actions" }, te = {
	class: "admin-services__section",
	...
	props: { client: {} },
	setup(e) {
		let v = e, F = y("apiBase", ""), I = d(() => typeof F == "string" ? F : F?.value ?? ""), L = new l(v.client ?? new n({
			baseUrl: I.value,
			tokenStore: new r()
		})), R = o(), z = T(), B = ee(), V = S(null), H = S(!0), U = S(null), W = S(!1), G = d(() => V.value?.configured === !1);
		async function K() {
			H.value = !0, U.value = null;
			try {
				V.value = await L.getTraktStatus();
			} catch (e) {
				U.value = t(e, "Failed to load Trakt status."), R.error(U.value);
			} finally {
				H.value = !1;
			}
		}
		...
		async function ce() {
			if (!W.value) {
				...
				try {
					...
					R.error(t(e, "Failed to disconnect Trakt."));
				} finally {
					W.value = !1;
				}
			}
		...
		async function Q() {
			...
			try {
				...
				Y.value = t(e, "Failed to load Last.fm status."), R.error(Y.value);
			} finally {
				J.value = !1;
			}
		}
		...
		async function ue() {
			if (!X.value) {
				...
				try {
					...
					R.error(t(e, "Failed to disconnect Last.fm."));
				} finally {
					X.value = !1;
				}
			}
		...
		return b(() => {
			...
			h("section", D, [h("div", O, [t[0] ||= h("h2", {
				...
				_: 1
			}, 8, ["tone", "label"]))]), h("div", k, [H.value ? (x(), m("div", A, [_(s, {
				variant: "text",
				lines: 2
			})])) : V.value === null ? (x(), f(c, {
				key: 1,
				...
				V.value.connected && V.value.username !== null ? (x(), m("dl", j, [t[2] ||= h("dt", null, "Username", -1), h("dd", null, C(V.value.username), 1)])) : p("", !0),
				!V.value.connected && G.value ? (x(), m("p", M, [...t[3] ||= [
				...
				h("div", N, [V.value.connected ? (x(), f(i, {
					key: 1,
					variant: "outline",
					loading: W.value,
					onClick: ce
			...
			h("section", te, [h("div", ne, [t[5] ||= h("h2", {
				id: "lastfm-heading",
				class: "admin-services__section-title"
				...
				_: 1
			}, 8, ["tone", "label"]))]), h("div", re, [J.value ? (x(), m("div", ie, [_(s, {
				variant: "text",
				lines: 2
			})])) : q.value === null ? (x(), f(c, {
				key: 1,
				...
				q.value.connected && q.value.username !== null ? (x(), m("dl", ae, [
...
//# sourceMappingURL=ServicesPage-MSQAAoEH.js.map