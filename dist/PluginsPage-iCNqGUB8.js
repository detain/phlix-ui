...
import { c as n, f as r, t as ee } from "./client-DbgRjcPy.js";
...
import { t as s } from "./Modal-DEuPeh-g.js";
import { t as ne } from "./Skeleton-DkSoWF3C.js";
import { t as c } from "./EmptyState-B2QnGIQT.js";
import { i as re, r as l, t as ie } from "./plugins-DLguuSHb.js";
...
var pe = {
	...
	props: { client: {} },
	setup(e) {
		let ae = e, C = oe("apiBase", ""), He = d(() => typeof C == "string" ? C : C?.value ?? ""), w = new ie(ae.client ?? new ee({
			baseUrl: He.value,
			tokenStore: new n()
		...
		async function O() {
			...
			try {
				...
				D.value = r(e, "Failed to load plugins."), T.error(D.value);
			} finally {
				E.value = !1;
			}
		}
		...
		async function A() {
			...
			try {
				...
				T.error(r(e, "Failed to load the plugin catalog."));
			} finally {
				We.value = !1;
			}
		}
		...
		async function at() {
			...
			try {
				...
				let t = l(e);
				T.error(t === "plugin.catalog.url.invalid" ? "That catalog URL is not valid (use an http(s):// URL)." : r(e, "Failed to add catalog."));
			} finally {
				nt.value = !1;
			}
		}
		async function ot(e) {
			if (z.value === null) {
				...
				try {
					...
					T.error(r(e, "Failed to remove catalog."));
				} finally {
					z.value = null;
				}
			}
		...
		async function ut() {
			if (!V.value) {
				...
				try {
					...
					T.error(r(e, "Failed to check for updates."));
				} finally {
					V.value = !1;
				}
			}
		...
		async function pt() {
			if (!H.value) {
				...
				try {
					...
					T.error(r(e, "Failed to apply updates."));
				} finally {
					H.value = !1;
				}
			}
		...
		async function mt(e) {
			...
			try {
				...
				W.value = t, T.error(r(e, "Failed to change auto-update."));
			}
		}
		let K = y(null);
		async function ht(e, t) {
			if (K.value === null) {
				...
				try {
					...
					T.error(r(e, "Failed to update plugin."));
				} finally {
					K.value = null;
				}
			}
		...
		async function gt() {
			...
			if (e) try {
				...
				T.error(r(e, "Failed to uninstall plugin.")), q.value = null;
			}
		}
		...
		async function Tt(e) {
			...
			try {
				...
				T.error(r(e, "Failed to load plugin settings.")), J.value = null;
			} finally {
				_t.value = !1;
			}
		}
		...
		async function Dt() {
			...
			try {
				...
				let t = re(e);
				Object.keys(t).length > 0 ? (Q.value = t, T.error("Some settings could not be saved — check the highlighted fields.")) : T.error(r(e, "Failed to save settings."));
			} finally {
				vt.value = !1;
			}
		}
...
//# sourceMappingURL=PluginsPage-iCNqGUB8.js.map