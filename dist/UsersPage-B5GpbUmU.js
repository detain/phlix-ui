...
import { a as i, l as ee, n as te, s as ne, t as re } from "./users-BdXH0IYQ.js";
...
var _e = {
	...
	props: { client: {} },
	setup(e) {
		let de = e, S = fe("apiBase", ""), ft = l(() => typeof S == "string" ? S : S?.value ?? ""), C = new re(de.client ?? new r({
			baseUrl: ft.value,
			tokenStore: new t()
		})), w = ie(), pt = fe("phlixConfig", null), mt = l(() => pt?.app === "hub"), ht = l(() => ne.map((e) => ({
			value: e.value,
			label: e.label
		...
		async function mn() {
			...
			if (!e || !t) return;
			let r = ln(Z.value), i = ln(Q.value), ee = un($.value);
			if (r === null || i === null) {
			...
			let te = an.value, ne = X.value !== t.throttle_bps, re = String(Z.value) !== te.quotaInGiB || String(Q.value) !== te.quotaOutGiB || String($.value) !== te.maxStreams;
			if (!ne && !re) {
			...
			rn.value = !0;
			try {
				if (ne && await C.setThrottle(e.id, X.value), re) {
					let t = {
						quota_bytes_in: r,
						quota_bytes_out: i,
						max_concurrent_streams: ee
					};
		...
		return pe(E), (e, t) => (g(), f("section", _e, [
			...
			_t.value ? (g(), f("div", ye, [h(se, {
				...
				_: 1
			}, 8, ["description"])) : gt.value.length === 0 ? (g(), u(ce, {
				key: 2,
				icon: "user",
				title: "No users yet"
			}, {
...
//# sourceMappingURL=UsersPage-B5GpbUmU.js.map