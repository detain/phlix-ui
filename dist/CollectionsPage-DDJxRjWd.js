...
import { t as ee } from "./Badge-D9Tdn6WP.js";
import { t as a } from "./Modal-DLVWGUN9.js";
import { t as te } from "./useToastStore-BDoKlU6N.js";
import { n as o, t as s } from "./EmptyState-BEMIpc2l.js";
...
}, D = /*#__PURE__*/ t(/* @__PURE__ */ h({
	__name: "CollectionsPage",
	props: { client: {} },
	setup(t) {
		let h = t, D = ie("apiBase", ""), pe = l(() => typeof D == "string" ? D : D?.value ?? ""), O = new ne(h.client ?? new n({
			baseUrl: pe.value,
			tokenStore: new i()
		})), k = te(), A = _([]), j = _(!0), M = _(null);
		async function N() {
			j.value = !0, M.value = null;
			try {
				A.value = await O.list();
			} catch (t) {
				M.value = e(t, "Failed to load collections."), k.error(M.value);
			} finally {
				j.value = !1;
			}
		}
		...
		async function H() {
			...
			try {
				if (t) {
					let e = { name: I.value };
					await O.update(t.id, e), k.success("Collection updated.");
				} else {
					let e = {
						name: I.value,
						library_id: L.value
				...
				k.error(e(t, "Failed to save collection."));
			} finally {
				R.value = !1;
			}
		}
		...
		async function he() {
			let t = U.value;
			if (t) try {
				await O.remove(t.id), k.success("Collection deleted."), U.value = null, await N();
			} catch (t) {
				k.error(e(t, "Failed to delete collection.")), U.value = null;
			}
		}
		...
		async function X(t) {
			K.value = !0;
			try {
				G.value = (await O.get(t)).items;
			} catch (t) {
				k.error(e(t, "Failed to load items."));
			} finally {
				K.value = !1;
			}
		}
		...
		async function ve(t) {
			let n = W.value;
			if (n) try {
				await O.removeItem(n.id, t.id), k.success("Item removed."), await X(n.id);
			} catch (t) {
				k.error(e(t, "Failed to remove item."));
			}
		}
		async function Q() {
			let t = W.value;
			if (t) {
				...
				J.value = !0;
				try {
					await O.bulkAdd(t.id, q.value), k.success("Items added."), q.value = "", await X(t.id);
				} catch (t) {
					k.error(e(t, "Failed to bulk-add items."));
				} finally {
					J.value = !1;
				}
			}
		...
		async function ye(t) {
			try {
				await O.refresh(t.id), k.success("Collection refreshed."), await N();
			} catch (t) {
				k.error(e(t, "Failed to refresh collection."));
			}
		}
		function $(e) {
		...
		return ae(N), (e, t) => (g(), d("section", w, [
			...
			m(a, {
				...
				default: x(() => [K.value ? (g(), d("div", ue, [m(o, {
					...
					scope: "col",
					class: "admin-collections__actions-col"
				}, "Actions")])], -1), f("tbody", null, [(g(!0), d(c, null, v(G.value, (e) => (g(), d("tr", { key: e.id }, [f("td", null, y($(e)), 1), f("td", null, [m(r, {
					variant: "ghost",
					size: "sm",
					"left-icon": "x",
					"aria-label": `Remove ${$(e)}`,
...
//# sourceMappingURL=CollectionsPage-DDJxRjWd.js.map