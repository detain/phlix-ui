...
var T = {
	...
	props: { client: {} },
	setup(e) {
		let v = e, j = y("apiBase", ""), M = d(() => typeof j == "string" ? j : j?.value ?? ""), N = new l(v.client ?? new t({
			baseUrl: M.value,
			tokenStore: new i()
		...
		async function V() {
			...
			try {
				...
				L.value = r(e, "Failed to load DLNA server status."), P.error(L.value);
			} finally {
				I.value = !1;
			}
		}
		async function H() {
			if (!R.value) {
				...
				try {
					...
					P.error(r(e, "Failed to start DLNA server."));
				} finally {
					R.value = !1;
				}
			}
		...
		async function U() {
			if (!R.value) {
				...
				try {
					...
					P.error(r(e, "Failed to stop DLNA server."));
				} finally {
					R.value = !1;
				}
			}
...
//# sourceMappingURL=DlnaServerPage-DZH7wet6.js.map