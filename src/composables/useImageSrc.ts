...
import { useMediaApiBase } from './useApiBase';
import { resolveImageSrc, resolveImageSrcset } from '../utils/imageSrc';
 ...
 */
export function useImageSrc(): ImageSrcResolvers {
  const base = useMediaApiBase();
  return {
    imgSrc: (url) => resolveImageSrc(base.value, url),
    imgSrcset: (srcset) => resolveImageSrcset(base.value, srcset),
  };
}
