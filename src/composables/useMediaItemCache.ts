...
export const MEDIA_CACHE_TTL_MS = 60_000; // 60 seconds
...
export function getMediaItemCacheEntry(id: string): CachedMediaItem | undefined {
  return mediaItemCache.get(id);
}

/** True when `entry` exists and is younger than the TTL relative to `now`. */
export function isMediaItemCacheFresh(
  entry: CachedMediaItem | undefined,
  now: number = Date.now(),
): boolean {
  return entry !== undefined && now - entry.ts < MEDIA_CACHE_TTL_MS;
}

/** Store/refresh `item` under `id`, stamping the cache time (`ts`). */
export function cacheMediaItem(id: string, item: MediaItem, ts: number = Date.now()): void {
  mediaItemCache.set(id, { item, ts });
}
 ...
 */
export function clearMediaItemCache(): void {
