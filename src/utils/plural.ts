...
/**
 * `Intl.PluralRules` construction is not free and the same few (locale, type)
 * pairs are used over and over during a render, so instances are memoised. The
 * cache is keyed by locale+type and is bounded by the number of distinct locales
 * the page actually uses — in practice one — so it cannot grow without bound.
 */
const rulesCache = new Map<string, Intl.PluralRules>();

/**
 * Cache-key separator between the locale and the plural type.
 *
 * ⚠ **Honest scope: this does NOT currently prevent a reachable bug.** The obvious
 * story — "plain concatenation would let `('en-cardinal', undefined)` collide with
 * `('en', 'cardinal')`" — is *false*, and was written here before it was checked.
 * `type` is exactly two fixed strings, so a collision would need
 * `A + 'cardinal' === B + 'ordinal'`; equal length forces `B = A + c`, which
 * reduces to `'ardinal' === 'ordinal'`. It is arithmetically impossible, and a
 * brute force over all locale strings up to length 4 finds no collision either.
 * An invalid `type` from untyped JS cannot sneak in, because `Intl.PluralRules`
 * throws `RangeError` on construction, *before* the cache write.
 *
 * It is kept as defence-in-depth for the case this reasoning stops holding — a
 * third key component, or `type` widening beyond two values — since it costs one
 * character and the failure it would guard is a silent wrong-locale answer.
 * `plural.test.ts` pins the property that IS testable: the key incorporates BOTH
 * components, proven by mutation (drop either one and a test goes red).
 *
 * Written as the ESCAPE `\x00`, never as a raw 0x00 byte in the source. A literal
 * NUL makes git classify the whole file as binary — `git diff` then shows
 * `Bin 0 -> N bytes` instead of a patch, so every later change to this file is
 * unreviewable and unblamable, and editors/formatters can truncate at the NUL.
 * That is exactly what happened here, and it was caught in review rather than by
 * any gate. `\x00` rather than `\0`, because `\0` followed by a digit is a legacy
 * octal escape.
 */
const CACHE_KEY_SEPARATOR = '\x00';

function rulesFor(options?: PluralOptions): Intl.PluralRules {
  const locale = options?.locale;
  const type = options?.type ?? 'cardinal';
  const key = `${locale ?? ''}${CACHE_KEY_SEPARATOR}${type}`;
 ...
 */
export function pluralCategory(count: number, options?: PluralOptions): PluralCategory {
  if (!Number.isFinite(count)) return 'other';
  return rulesFor(options).select(count) as PluralCategory;
}
...
export function plural(count: number, forms: PluralForms, options?: PluralOptions): string {
  const category = pluralCategory(count, options);
  const chosen = forms[category];
  return chosen === undefined ? forms.other : chosen;
}
...
export function pluralize(
  ...
  return plural(count, { one, other }, options);
}
...
export function pluralCount(
  ...
  const n = options?.formatNumber ? count.toLocaleString(options.locale) : String(count);
  return `${n} ${pluralize(count, one, other, options)}`;
}
 ...
 */
export function selectPluralTemplate(
  template: string,
  count: number,
  options?: PluralOptions,
): string {
  ...
  const available = new Set(rules.resolvedOptions().pluralCategories);
  const slots = PLURAL_CATEGORIES.filter((c) => available.has(c));

  const forms: Partial<Record<PluralCategory, string>> = {};
  slots.forEach((category, i) => {
    forms[category] = parts[Math.min(i, parts.length - 1)];
  });
  // `other` exists in every locale, so `slots` always assigned it; the `??` is a
  // belt-and-braces fallback for an engine reporting an `other`-less category set.
  return plural(count, { ...forms, other: forms.other ?? parts[parts.length - 1]! }, options);
}
