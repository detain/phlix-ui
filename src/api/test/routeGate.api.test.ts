...
describe('route gate — vendored contracts export (S280 re-adoption)', () => {
     ...
     * contracts `59fd9b02`, and the sha pin is the server-currency proof.
     */
    const VENDORED_MANIFEST_MD5 = 'b6acafdf0d4c6a31b8c258631df30b73';
    const VENDORED_MANIFEST_SERVER_SHA = 'df6aa8e5cf577fa4a6fa9d73bcad7e7ec7d4fc4a';
...
describe('route gate — syncplay.ts (SyncPlayApi)', () => {
    ...
    it('issues 5 distinct urls (7 calls), every one a registered server route', async () => {
        ...
        await driveGated(server, 'createRoom', () => api.createRoom({ name: 'Movie Night' }));
