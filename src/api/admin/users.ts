 ...
 */
export const PROFILE_LAST_ERROR_CODE = 'profile.last_profile';

/** Human-readable text the server pairs with {@link PROFILE_LAST_ERROR_CODE}. */
export const PROFILE_LAST_ERROR_TEXT = 'Cannot delete the last profile';

/** Lane marker (S465 merge ritual) — code-resident literal, not a comment. */
export const S465STUBFULL409X9K6 = 'faithful-last-profile-409';
...
export class AdminUsersApi {
  ...
  updateProfileSchedule(
    ...
    daysOfWeek: string[],
    isActive: boolean,
  ): Promise<{ schedule: AccessSchedule | never[]; message: string }> {
    return this.client.put<{ schedule: AccessSchedule | never[]; message: string }>(
