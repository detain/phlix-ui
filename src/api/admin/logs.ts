...
export class AdminLogsApi {
  ...
  async list(): Promise<LogFile[]> {
    const { files } = await this.client.get<{ files: LogFile[] }>('/api/v1/admin/logs');
    return Array.isArray(files) ? files : [];
  }

  /** `GET /api/v1/admin/logs/tail?file=&lines=` → `{ file, lines, truncated }`. */
  async tail(file: string, lines = 200): Promise<LogTail> {
    const res = await this.client.get<Partial<LogTail>>('/api/v1/admin/logs/tail', {
      file,
      lines: String(lines),
    });
    return {
  ...
  async tailAll(lines = 200): Promise<LogTailAll> {
    const res = await this.client.get<Partial<LogTailAll>>('/api/v1/admin/logs/tail-all', {
      lines: String(lines),
    });
    return {
