 ...
 */
export const ADMIN_UPDATES_CHECK_ENDPOINT = '/api/v1/admin/updates/check';
...
export class AdminUpdatesApi {
   ...
   */
  async check(signal?: AbortSignal): Promise<CoreUpdateStatus> {
    const raw = await this.client.post<unknown>(ADMIN_UPDATES_CHECK_ENDPOINT, {}, signal);
    return parseCoreUpdateStatus(raw);
  }
}
